...
Suppose $y$ is an optimal schedule. Whenever we have $y_{i} + \rho \geq r_{j}$
for some conjunctive pair $(i,j) \in \mathcal{C}$ , we must have
$y_{i} + \rho = y_{j}$ by Proposition~\ref{prop:exhaustive}. In order to model
this rule, we first introduce a binary variable $\beta_{ij}$ that satisfies
\begin{align*}
...
We could add further cutting planes on the disjunctive decision variables,
because whenever $\beta_{ij} = 1$, the directions of the disjunctive arcs $i \rightarrow k$
...
%The number of disjunctive variables in the above formulation is of order $O(|\mathcal{N}|^{2})$.
%The problem is that this brings a lot of redundancy, since the decision to schedule $j$ immediately after $i$ involves setting $O(\mathcal{N})$ disjunctive decision variables.
%%
%We wonder whether there are alternative formulations that allow more efficient solving.

%\subsubsection*{Custom branch-and-bound}
%Instead of trying to find an efficient formulation and relying on the
%capabilities of modern MILP solvers, we can consider a tailored branch-and-bround algorithm.

\subsubsection*{Running time analysis}

% uses knitr to generate nice tables from raw running times measurements in .csv file
\input{results-woven.tex}
