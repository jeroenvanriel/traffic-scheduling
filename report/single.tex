...
%
\begin{align*}
  \tau(\mathcal{X}) = \arg\min_{\tau \geq 0} \sum_{s \in \mathcal{X}} \textrm{obj}(y_{\tau}(s)) .
\end{align*}
...
\begin{figure}
  ...
  \caption{Schematic overview of the neural heuristic. The distribution over the
    next route is parameterized as
    $p_{\theta}(\eta_{t+1} | s_{t}) = f(H(s_{t}))$, which is computed from the
    current route horizons $h$ via embedding $\bar{h}$ and the cyling trick to
    obtain $H$. Observe that the particular cycling shown in the figure
    corresponds to a situation in which the current route is
...
% reward definition
The reinforcement learning paradigm requires the definition of a reward.
%
...
\begin{align*}
  \nabla \mathbb{E}_{p_{\theta}} G_{0} &\propto \sum_{s} \mu(s) \sum_{a} q_{p_{\theta}}(s, a) \nabla p_{\theta}(a | s) \\
  &= \mathbb{E}_{p_{\theta}}\left[ \sum_{a} q_{p_{\theta}} (S_{t}, a) \nabla p_{\theta} (a | S_{t}) \right] \\
  &= \mathbb{E}_{p_{\theta}}\left[ \sum_{a} p_{\theta}(a | S_{t}) q_{p_{\theta}} (S_{t}, a) \frac{\nabla p_{\theta} (a | S_{t})}{p_{\theta} (a | S_{t})} \right] \\
  &= \mathbb{E}_{p_{\theta}}\left[ q_{p_{\theta}} (S_{t}, A_{t}) \frac{\nabla p_{\theta} (A_{t} | S_{t})}{p_{\theta} (A_{t} | S_{t})} \right] \\
  &= \mathbb{E}_{p_{\theta}} \left[ G_{t} \log \nabla p_{\theta} (A_{t} | S_{t}) ] .
\end{align*}
% \begin{align*}
