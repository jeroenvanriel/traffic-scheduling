...
next vehicle in the current lane, we will now consider the earliest arrivals
times of all unscheduled vehicles across lanes.
%
Because we want to tune the heuristic based on problem instances, we will
formulate it as a model $p_{\theta}(\eta_{t+1} | s_{t})$ of the conditional
distribution over the next route, given the current partial order, where
...
to avoid cluttering the notation and let $k(r)$ again denote the number of
...
$h(r)$ is simply transformed into a fixed-length embedding using a plain Elman
...
instance size. More precisely, from now on we use the objective defined as
\begin{align*}
  \textrm{obj}(y(s)) = \frac{1}{|\mathcal{N}|} \sum_{i \in \mathcal{N}} y_{i}(s) - a_{i} .
\end{align*}
%
Ideally, we want to report the average \textit{optimality gap} of each heuristic, which is defined as
\begin{align*}
...
%
Again, we use a fixed time limit per instance to bound the total computation
time of the analysis. Therefore, we cannot compute $y^{*}(s)$ using the
branch-and-cut procedure for large instances.
%
Instead of reporting the real optimality gap $\alpha_{\text{approx}}$, we report
the relative optimality gap, where we use the best solution that branch-and-cut
obtains within the time limit as proxy for $y^{*}(s)$.

The heuristics are fitted to the training data without much manual tweaking, but
...
%
For the neural heuristic, we plot the loss during training, see
Figure~\ref{fig:neural_fit}.

\input{data-comparison/comparison-woven.tex}
...
\begin{figure}
  ...
  \caption{Charts to verify neural heuristic model fit. For each indicated
    instance training set, the training loss is plotted over all epochs. Recall
    that the loss is defined as the average binary cross entropy loss.}
