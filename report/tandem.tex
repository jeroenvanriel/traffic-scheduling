...
\begin{remark}\label{rem:conditions}
  ...
  requirement \textit{(iii)} is violoated.
  %
  Condition (C2) is not necessary, which can for example be seen from the
  stopping trajectory in Figure~\ref{fig:proof} satisfying the conditions
  (in grey), which would have been valid even if $\gamma_{2}$ ended somewhat
  earlier than $t_{f}$, for example until the open dot.
\end{remark}
