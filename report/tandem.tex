...
\begin{define}\label{def:stopping-trajectory}
  ...
  $\xi$, from which it starts decelerating to a full stop, so that at time
...
\begin{lemma}\label{lemma:curvejoining}
  ...
  \begin{enumerate}[leftmargin=5em]
    \item[(C1)\quad] $\gamma_{2} \geq \gamma_{1}[a_{1}]$,
    \item[(C2)\quad] $b_{2} \geq t_{c} + \dot{\gamma}_{1}(t_{c}) / \omega$,
  \end{enumerate}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.9]{figures/motion/rough/curvejoiningproof}
  \caption{Sketch of the proof of Lemma~\ref{lemma:curvejoining} with some
    stopping trajectories drawn in grey.}%
  \label{fig:proof}
\end{figure}
...
\begin{outline}
  ...
  necessarily connected (see Figure \dots), so it is the union of a sequence of
  disjoint closed intervals $T_{1}, T_{2}, \dots, T_{n}$.
  ...
  $s(t)$ actually exists in both cases, because there is some $\xi$ such that
  $\gamma_{2}(t) = \gamma_{1}[\xi](t)$ and thus $s(t) = g(t, \gamma_{2}(t)) = \dot{\gamma}_{1}[\xi](t)$
  exists.
  ...
  \1 If $i \geq 2$, then $\varphi_{i} > \gamma_{2}$ somewhere. Let $i \geq 1$,
  we show that $\varphi_{i+1}(t) > \gamma_{2}(t)$ for some $t$.

  \2 Recall the lower bound property, so $\gamma_{2}(t) \geq \varphi_{i}(t)$ and
  $\dot{\gamma}_{2}(t) \geq \dot{\varphi}_{i}(t)$ for $t \geq \tau_{i}$.

  \2 Define $\hat{\tau}_{i} := \max \, T_{i}$, such that
  ...
  \end{align}
  since otherwise $\gamma_{2} = \varphi$ on some open neighborhood of
  $\hat{\tau}_{i}$ and thus also
  \begin{align}
    \dot{\gamma}_{2}(t) = \dot{\varphi}_{i}(t) = g(t, \varphi_{i}(t)) = g(t, \gamma_{2}(t)), \quad \text{ (recall \eqref{eq:phi-tangent}) }
  \end{align}
  which contradicts the definition of $\hat{\tau}_{i}$.

  \2 Therefore, we have $\gamma_{2}(t) > \varphi_{i}(t)$ for all $t \geq \hat{\tau}_{i} + \delta$. For $t = \tau_{i+1}$ in particular, it follows that
  \begin{align}
    \varphi_{i+1}(\tau_{i+1}) = \gamma_{2}(\tau_{i+1}) > \varphi_{i}(\tau_{i+1}) ,
  \end{align}
  which shows that $\varphi_{i+1} > \varphi_{i}$ on $(\xi_{i}, \infty)$, due to
  Property~\ref{prop:xi-unique}.

  \2 Since $\xi_{i+1} < t_{c}$, we have
  $\varphi_{i+1}(\xi_{i+1}) < \gamma_{2}(\xi_{i+1})$, but also
  $\varphi_{i+1}(\tau_{i}) > \varphi_{i}(\tau_{i}) = \gamma_{2}(\tau_{i})$.
  Therefore, there must be some $\tau^{*} \in (\xi_{i+1}, \tau_{i})$, such that
  $\varphi_{i+1}(\tau^{*}) = \gamma_{2}(\tau)$ and
  $\dot{\varphi}_{i+1}(\tau^{*}) > \dot{\gamma}_{2}(\tau^{*})$, so
  $\varphi_{i+1}$ crosses $\gamma_{2}$ at $\tau^{*}$.
  ...
  \2 Since $\dot{\gamma}_{2}(\tau_{0}) < \dot{\varphi}_{i}(\tau_{0})$, there
  must be some $t \in (\tau_{0}, \tau_{i})$ such that
  $\dot{\gamma}_{2}(t) > \dot{\varphi}_{i}(t)$.

  \2 Since
  $\dot{\gamma}_{2}(\tau_{0}) < s(\tau_{0}) = \dot{\varphi}_{i}(\tau_{0})$ and
  $s(t) \leq \dot{\varphi}_{i}(t)$ for $t \in [\tau_{0}, \tau_{i}]$, this means
  there is some $\tau^{*}$ such that $\dot{\gamma}_{2}(\tau^{*}) = s(\tau^{*})$,
  again as a consequence of the intermediate value theorem. Therefore,
  $\tau^{*} \in T_{j}$ for some $j < i$, which shows that $i \geq 2$.


  \1 In conclusion, the above two points establish that
  $\varphi_{i} \leq \gamma_{2}$ if and only if $i=1$, so that $\varphi := \varphi_{1}$ is
  the unique trajectory satisfying the stated requirements.
  \qedhere
\end{outline}
\end{proof}
...
\begin{remark}
  Note that condition (C1) in Lemma~\ref{lemma:curvejoining} is necessary, because Figure \dots shows
  a configuration in which the trajectories cannot be joined. Condition (C2)
