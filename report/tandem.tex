...
\begin{figure}
  \centering
  \includegraphics[scale=1]{figures/motion/trajectories}
...
\begin{figure}
  \centering
  \includegraphics[scale=1.0]{figures/motion/necessary-conditions}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=1.1]{figures/motion/multiple-discontinuities}
...
We now return to the lane planning problem. Given some vector of starting
times $a = (a_{1}, \dots, a_{N})$ and departure times
$b = (b_{1}, \dots, b_{N})$, we want to find
\begin{subequations}
\begin{alignat}{3}
...
However, since $\dot{x}_{i-1}(b_{i-1}) = 1$, we can extend \comment{Let's leave
  ...
  burden.} the boundary until $b_{i-1} + L$ by defining
...
{\color{Navy} Explain what we mean with ``smoothing times''.}
...
\end{align*}
%
{\color{Navy} Argue why the other combinations are not possible.}

We will now examine the remaining four cases in which two of these partial
trajectories can actually be joined by a deceleration part. We will later use these
results to characterize optimal trajectories for our optimal control problem.

\comment{The upcoming derivations are very tedious, so will be moved to appendix
  eventually.}
