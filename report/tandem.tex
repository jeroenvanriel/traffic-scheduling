...
\newpage
\note{We might need this later\dots}
...
%
We will show that feasibility is equivalent to $a_{i}$ and $b_{i}$ satisfying a
certain system of inequalities.

\begin{assump}[Waiting position]\label{assump:L-bound}
  Assume that $L < B - A$.
\end{assump}
...
\note{Why do we assume $a \geq c$ and $b \geq d$ in the next lemma? Because we
  assume $a_{1} \leq a_{2} \leq \dots \leq a_{N}$ and similarly for $b_{i}$.}
...
$b = (b_{1}, b_{2}, \dots, b_{N})$ are ordered as
$a_{1} \leq a_{2} \leq \dots \leq a_{N}$ and
...
\begin{proof}
  ...
  We show ``(C2) $\implies$ (C3)''. (simple)

  We show ``(C3) $\implies$ (C2)''. (induction)

  We show ``(C?) $\implies$ (C4)''. (necessity)

  We show ``(C4) $\implies$ (C?)''. (sufficiency)
\end{proof}
