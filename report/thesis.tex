...
\usepackage{amsmath,amsfonts,amssymb,amsthm,thmtools,thm-restate,cleveref}
...
\author{Jeroen van Riel \\[1em]} %{Combined Master's Thesis \\[1em] Industrial and Applied Mathematics \\ Computer Science and Engineering \\[-0.1em] --- \\[-0.1em] Eindhoven University of Technology}}
\date{\monthyeardate\today}
\newcommand{\mytitle}{\huge Machine Learning-Based Scheduling for Coordination of Automated Vehicles}
\title{\mytitle}
...
\begin{minipage}[h]{\textwidth}
  \note{[ everything in blue is a comment or ``pending change'' ]}
    \maketitle
    \begin{center}
    \textbf{Abstract}
    \end{center}
...
\paragraph{Discrete decisions in traffic coordination.} We need to take a
...
\note{[probably going to change] The rest of this chapter discusses some preliminaries: we briefly discuss the
...
\1 fully automated systems
  \2 We will use the term (connected) Automated Vehicles, shorthand AVs, because
    we "take communication for granted"
  \2 already applicable in "confined sites" like
    \3 ports, mines, quarries
    \3 warehouses
    \3 manufacturing sites (e.g. Prodrive)
    \3 indoor vertical farming sites (e.g. Helmond)
\1 focus on the optimization-based coordination of vehicle motion (joint="decisions+control")
  \2 lower-level steering control of vehicles is ignored: it is assumed that
    vehicles can follow planned trajectories perfectly
\1 why such stylized models? /(chatGPT suggestion: this also touches on challenges)/
  \2 better understand inherent difficulties
  \2 obtain results that are better interpreted:

    "Simple models are relevant, because they provide upper bounds on what we
    can expect to gain in more realistic situations with decentralized control,
    communcation latency and noisy information."
\end{outline}

\paragraph{Goals.}

Understand how to \emph{integrate} model-free (RL) with model-based (opt. control, scheduling) algorithms.

(optimization setting allows us to focus on goals that can be defined mathematically)

\begin{outline}
\1 conflicting optimization goals:
  \2 min. delay (max. throughput), like in studied in queueing theory
  \2 min. energy consumption / reduce emission
  \2 fairness of usage (think reservation / bidding systems)
  \2 guarantee safety (need to define what this means)
\end{outline}

\paragraph{Challenges: [REVIEW]}
\emph{(ChatGPT says: "It’s long and risks overwhelming the reader before they even get to the fun methods.")}

\emph{Emphasize the combinatorial problems that occur naturally.}

\begin{outline}
\1 broad modeling landscape
  \2 mention some aspects (preview):
    \3 network topology
    \3 vehicle dynamics: degree of realism vs. computational tractability
    \3 random events, e.g., arrivals, failure models
    \3 topic on its own: communication systems (protocols, delay, robustness to failure)
  \2 no standard benchmarks yet like Taillard (they said this in the poorly
    written survey, but is this really so?)

\1 inherent algorithmic challenges (even in stylized deterministic models)
  \2 mix of discrete decisions and continuous-time control of vehicles (almost
    always non-convexity involved)
  \2 even when routes are fixed, there is still a lot of freedom in determining the
    speed profile over the course of routes. (infinite-dimensional optimization =
    optimal control)

\1 random network effects
  \2 inherent nonstationarity of mobility demand, with fluctuations
    throughout the day, makes requires adaptive control.
  \2 decisions at one intersection directly influence the arrival process at
    nearby intersections
  \2 in dense urban networks, these interactions are highly complex: a lot of
    effort has gone into developing methods for predicting traffic

\1 practical constraints /(for transfer to applications)/
  \2 real-time computation requirements (fast)
    (similar setting in which MPC is often employed)
  \2 real-time communication requirements (robust)
\end{outline}

\paragraph{Main methodologies (overview).}
\emph{(...and introduce our contributions at this point?)}

\begin{outline}
\1 bilevel decomposition -> scheduling -> MILP
  \2 observation: the combinatorial part of offline trajectory coordination shows similarities
    to job-shop scheduling

  \2 motivation: bilevel decomposition can provide a conceptual tool for further
    development of algorithmic ideas

  \2 goal: (systematic/interpretable) dimensionality reduction of the problem
    \3 for example, optimal control problem: discretization, splines
    \3 integer programming: benders' decomposition
    \3 why "systematic/interpretable": because e.g. model-free RL is not
      interpretable, so does provide less insight for further algorithm design

  \2 question: when is such a dimensionality reduction sound?

    i.e., when can the combinatorial part be completely separated from the
    continuous-time control problem

  \2 subquestion: given this decomposition, can the feasibility of lower-level be
    easily characterized or approximated?

\1 "ML for algo learning" -> sequential solution encoding/synthesis -> sequence
  modeling -> RL for param. fitting
  \2 observation: new perspective, recent advances
  \2 "automated heuristic search" inspiration: exploiting specific structure in the problem
    \3 key concept: algorithm as MDP
  \2 motivation: recent successes in neural combinatorial optimization (e.g. TSP)
  \2 question: how to apply this methodology in traffic coordination setting
    \3 subquestion: How do parametric sequence models perform on this task?
    \3 subquestion: How to parameterize the model?
\end{outline}

\paragraph{Scope of literature review.}
"We do a general literature review on
\emph{optimization-based methods for traffic coordination in fully automated mobility
systems}. Brief introduction of \emph{methodologies} in the chapter where we first need
it. We can also put an overview of literature there, or in the appendix."

\paragraph{Adjacent topics.}
\emph{(so don't spend too much time here)}
\emph{(present in order: least relevant/history to slightly adjacent)}

\begin{outline}
\1 dynamic path/route planning
\1 focus on decentralized nature -> communication becomes inherent issue
  \2 reservation systems
  \2 auction/bidding systems
  \2 fairness: average vehicle delay vs. all vehicles experience similar delay
    (e.g., in proportion to the length of their trip)
\1 traffic signal optimization (/"brief for historical context"/)
  \2 static (Fleuren)
  \2 adaptive (baroque RL studies)
\1 platooning in single lane, CACC
\1 platooning and CACC more generally: trajectory optimization, given fixed signal phases
\end{outline}

special remark: why not model-free RL?
\begin{outline}
\1 model-free RL ("brute-force" control synthesis)
  \2 recognize that this is a powerful framework
\1 in essence, we advocate to use model-free RL, but at a more local level (to,
  hopefully, exploit structure)
\end{outline}

\paragraph{topics / distinguishing features:}

\begin{outline}

\1 modeling aspects

\2 modeling of conflict zones "mutex zones" (see Kojchev)
\2 single int. vs network
\2 single lane vs multilane
\2 vehicle dynamics model
\2 use of simulator like SUMO and VISSIM

\2 central vs decentralized control
\2 mixed traffic (legacy vehicles, human drivers)

\1 /specific focus on model/challenges/algorithmic approach/

\2 focus on continuous control:
  \3 trajectory optimization, given fixed signal phases
    \4 different objectives:
      \4 minimizing fuel consumption
      \4 maximizing passenger comfort

  \3 platooning
    \4 is widely used as the basis for heuristic approaches
    \4 arises as a result of the assymetry of the combinatorial problem, namely the
       difference in processing time and switch-over time
    \4 cooperative adaptive cruise control (CACC), string stability

  \3 important in all these: real-time control, which causes the following common issues
    \4 how to actually implement this in electronic/software controller?
    \4 discretization
    \4 stability
    \4 decentralization causes problems: communication / synchronization (string stab.)

\2 focus on stochasticity: random arrivals and failure
  \3 -> offline vs online optimization
  \3 "rolling horizon" schemes
    \4 Model Predictive Control (MPC)
  \3 robustness to failures, safety guarantees even without explicit "fatal
    crash"-type failures happening (e.g., Miculescu & Karaman: Theorem IV.5)

\2 focus on combinatorics:
  \3 of course: service order at intersections
  \3 "MUTEX" zones, in general, see Kojchev
  \3 even: in which order to organize vehicles in a platoon approaching
    intersection (efficiently "voorsorteren")
  \3 larger scale: path planning in network
  \3 common methods:
    \4 dealing with binary choices: Mixed-Integer (Linear) Programming
    \4 heuristics and local search
    \4 classical search algorithms like A*
\end{outline}

\paragraph{main topics/features for literature comparison table:}
\begin{outline}
\1 model/challenge focus: "combinatorics", "cont-time control", "decentralized control"
\1 main algorithmic paradigm: e.g. "reservation", "decomposition", "MILP", "model-free RL"
\end{outline}
...
%
Finally, we briefly suggest a local search heuristic.
...
\begin{remark}
  ...
  \note{Although the theory for dealing with state constraints (\`a la
    ...
    dealt with in numerical approaches, often using some sort of penalty-based
    encoding.}
\end{remark}
...
\end{alignat}
Our main interest is in solving this problem for different instances of $s$, the
...
\paragraph{Explicit intersection crossing.}
...
\begin{eg}
  ...
  satisfying the assumption~\eqref{eq:room-assumption}, such that it can perform a full deceleration
  ($\ddot{x}_{i} = \omega$) and come to a stop somewhere before the start of the
  intersection at some point $x_{i}(1/\omega) < B$. Consider the cost functional
...
%
To make this simpler, we additionally assumptions that all vehicles start at
full speed $v_{i}^{0} = \bar{v}$ and that vehicles must enter the intersection
at full speed, i.e., we add the constraint $\dot{x}_{i}(\tau(x_{i})) = \bar{v}$
to~\eqref{eq:single-trajectory-optimization-variant} and $\dot{x}_{i}(y_{i}) = \bar{v}$ to~\eqref{eq:lower-level}. \note{[do we need this?]} Let
the resulting problems variants be denoted (T**) and (L**), respectively
\comment{I know, this is not the prettiest; I will think about a different way
  of writing this.}

Under these assumptions, the set of feasible parameters $\mathcal{T}(s_{r})$ is
now easier to characterize.
%
Given some $(y,z)$, every $x_{i}$ in a a solution $x_{r}$ for~(L**) satisfies
$x_{i}(y_{i}) = \bar{v}$. Therefore, we can let each vehicle continue at full
speed across the intersection, without inducing extra cost. This takes exactly
...
%
Since $z_{i}$ is not involved in $J(\cdot)$, this means we only have to care about finding
\begin{align}
  \label{eq:1}
...
\end{alignat}
\end{subequations}
This is a typical scheduling problem, which can for example be solved within the
mixed-integer linear programming framework after encoding the
\textit{disjunctive constraints}~\eqref{eq:disjunctive} using the big-M
technique. This methodology will be applied in the next section.


\section{Crossing time scheduling}
\label{sec:branch-and-cut}

The conclusion of the previous section is that, after making certain assumptions
and adding some constraints, the trajectory optimization problem~\eqref{eq:single-trajectory-optimization} reduces to
a scheduling problem~\eqref{eq:crossing_time_scheduling} of finding a crossing time schedule $y$ that determines
the time of crossing the intersection for each vehicle in the sytem.
%
...
We will illustrate both types of methods in the rest of this section.
%
First, we show how to leverage the general branch-and-cut framework by
formulating~\eqref{eq:crossing_time_scheduling} as a mixed-integer linear program and defining three types of
cutting planes.
%
We evaluate the computational performance of this method in Section~\ref{sec:runtime}.
\comment{Clearly motivate why we want fast heuristics.} As an illustration of a
classical approach to heuristic design, Section~\ref{sec:local_search} provides a brief description
of a local search procedure.
%
The next chapter will discuss methods to automate the search for heuristics
...
\note{ \emph{strenghts:} optimality guarantee, flexibility: fairly broad
  modeling capacity, maturity: take advantage of modern solvers;
%
  \emph{weaknesses:} no guarantees on computation time, reformulation can be
  involved; }
...
%
A typical example of such problems is a mixed-integer linear program, where the
goal is to minimize
...
many types of scheduling problems, because it allows discrete choices to be
...
%
Moreover, reformulating a problem as~\eqref{eq:MILP} opens up the possibility to leverage
the collective effort that has gone into developing fast general solvers: many
modern solvers, e.g., Gurobi~\cite{gurobi} (commercial) or SCIP~\cite{BolusaniEtal2024OO} (academic), employ
specialized interal heuristics and techniques like cutting planes to derive
better bounds and thus achieve more pruning of the search tree to speed up the
solving process.

% The basis of solving~\eqref{eq:MILP} relies on a so-called branch-and-bound
% procedure. The main idea is to explore a search tree whose nodes correspond to
% progressively more constrained relaxations of the problem. Each time the tree
% branches at some node, the value of some integer variable is constrained as
% either $y_{j} \geq v$ or $y_{j} \leq v$ for some integer $v \in \mathbb{Z}$ and
% this constraint will propagate further into all subtrees of this node. The
% integer constraints for the remaining variables are relaxed, producing a linear
% program that can be solved efficiently. The solutions of these relaxations are
% used to guide the search: whenever the objective of the relaxation at some node
% is higher than that of the the currently best known feasible solution, the
% subtree at this node is pruned.


\paragraph{Reformulation.}
To formulate the crossing time scheduling
problem~\eqref{eq:crossing_time_scheduling} into~\eqref{eq:MILP}, we rewrite the
disjunctive constraints using the big-$M$ method by introducing a binary
decision variable $\gamma_{ij}$ for every conflict $\{i, j\} \in \mathcal{D}$, such that
for every $(i,j) \in \bar{\mathcal{D}}$, setting $\gamma_{ij} = 0$ corresponds
to choosing the crossing order $i \rightarrow j$ and $\gamma_{ij} = 1$
corresponds to $j \rightarrow i$.
%
To avoid redundant variables in a software implementation, it might be necessary
to impose some arbitrary ordering of the conflicting vehicles by defining the
index set
\begin{align*}
  \bar{\mathcal{D}} = \{ (i,j) : \{i,j\} \in \mathcal{D}, \; r(i) < r(j) \} .
\end{align*}
We now have the following reformulation of~\eqref{eq:crossing_time_scheduling}:
%
\begin{equation}\tag{C'}\label{eq:C'}
\renewcommand{\arraystretch}{1.2}
\begin{NiceArray}{ r l @{} >{{}}c<{{}} @{} l @{} }
  \displaystyle \min_{y} & \displaystyle \sum_{i \in \mathcal{N}} y_{i} - a_{i} \\
  \text{s.t. } \; & a_{i} \leq y_{i} && \quad \text{ for all } i \in \mathcal{N} , \\
  & y_{i} + \sigma \leq y_{j} && \quad \text{ for all } (i,j) \in \mathcal{C} , \\
...
\end{equation}
%
where $M > 0$ is some sufficiently large number. In this form, the problem can
be solved by any off-the-shelf MILP solver, for which a wide variety of tooling
is available. For example, we used the AMPL modeling language to write the above
formulation in a solver-agnostic specification and use the
...
Before we evaluate whether~\eqref{eq:C'} can be solved in reasonable amounts of time
%
\comment{We could also take the opportunity to introduce the disjunctive graph
  at this point. However, I find this ``single machine'' situation not as
  eluminating as the general job shop setting, so I think I actually prefer to
  have this in the first chapter.}
%
we present some insights into the structure of optimal solutions, which also
serves as the basis for defining three types of cutting planes that can be added
to~\eqref{eq:C'}, which might improve the solving time.

\note{[some lack of continuity and motivation here]}
Recall the intial ordering of vehicles due to the safe headway constraints
  and the definition of earliest arrival time $a_{i}$. Let $\mathit{pred}(i)$
denote the set of vehicles that arrive no later than $i$ on route $r(i)$ and let
$\mathit{succ}(i)$ denote those that arrive no earlier,
%
...
\end{subequations}
%
Consider some conflict pair $(i,j) \in \bar{\mathcal{D}}$ and suppose
$\gamma_{ij} = 0$, so $i$ crosses before $j$, then any feasible solution must satisfy
\begin{align*}
  \text{``} p \rightarrow q \text{''} \, \equiv \, \gamma_{pq} = 0 \quad \text{ for all } \, p \in \mathit{pred}(i), \, q \in \mathit{succ}(j) .
\end{align*}
\note{[include graphical illustration, motivating our later introduction of the
  disjunctive graph]} Using the big-$M$ method, we can equivalently encode this
condition as
\begin{align*}
  \sum_{\substack{p \in \mathit{pred}(i)\\ q \in \mathit{succ}(j)}} \gamma_{pq} \leq \gamma_{ij} M ,
\end{align*}
for every conflict $(i,j) \in \bar{\mathcal{D}}$, to which we refer as the \textit{transitive cutting
  planes}.

% Under the condition that $\rho_{i} = \rho$ and $\sigma_{i} = \sigma > \rho$ for all $i \in \mathcal{N}$,

Apart from the just described redundancy, which is inherent to the way conflicts
are encoded, we will now study some less obvious structure in the problem.
%
...
disjunctive decision variables, because whenever $\delta_{ij} = 1$, \note{[remove
  disjunctive-graph-speak here]} the directions of the disjunctive arcs $i \rightarrow k$
and $j \rightarrow k$ must be the same for every other vertex $k \in \mathcal{N}$.
Therefore, consider the following constraints
\begin{align*}
...
\note{[either keep it simple here, we only want to illustrate that MILP is no silver bullet; or already setup our problem instance classes, which we will use in the results of the next chapter]}
\note{[I think the first option is best, because then this dicussion is then closer to the actual tables in which these classes 'low', 'med', 'high' are actually used]}

\comment{This part needs some rework: lots of technical details and missing clear conclusions.}
...
%
We will now give an example of such a neighborhood.
...
\note{[this concept has not been introduced at this point!]}
...
%
The bilevel decomposition and approximation scheme is further detailed
in~\cite{hultApproximateSolutionOptimal2015,hultTechnicalReportApproximate}.
%
For a textbook introduction of optimal control theory from the perspective of
mathematical analysis, we recommend the book of
Liberzon~\cite{liberzonCalculusVariationsOptimal}. The focal point in their
presentation is the Pontryagin maximum principle, which provides neccessary
conditions for optimality for optimal control problems.
%
...
guaranteed to find an optimal solution, but as we illustrated in Section~\ref{sec:runtime},
...
%
After precisely stating the model parameterization, we will show how to use
randomly generated problem instances for model fitting.
%
...
\paragraph{Sequence modeling for schedules.}
...
program~\eqref{eq:C'} from Section~\ref{sec:branch-and-cut}. For each such optimal schedule, we can
...
\note{Probably better to introduce these examples earlier on, i.e., in single intersection chapter.}
%
...
\begin{remark}
  ...
  \end{align}
  Consequently, $\gamma^{*}$ is an optimal solution to the single vehicle
  optimal control problem
  \begin{align}
...
\begin{subequations}
\begin{alignat}{2}
  &x_{i} \in D[a_{i},b_{i}] && \quad \text{ for each } i \in \{1, \dots, N\} , \\
...
%
We will need the following technical assumption, which is very reasonable to
assume in practice.
...
%
The following lemma concludes the   summarizes what we have established so far.
...
\end{lemma}
\begin{proof}
  Of course, (C0) and (C1) are equivalent by definition. Note that equivalence
  of (C1) and (C2) is handled by Lemma~\ref{lemma:downshift}. \note{ We show
    ``(C2) $\implies$ (C3)''. (simple) We show ``(C3) $\implies$ (C2)''.
    (induction)}
\end{proof}

\paragraph{Simpler entry space constraints.}
\note{[we will not proof this, but at least derive $a^{*}$ and maybe sketch what we would need for the proof (why we find it difficult)]}

The goal of the remainder of this section is to get rid of the entry space
constraints and replace them with inequality constraints. More specifically, we
will show that the above statements are further equivalent to
\begin{enumerate}[leftmargin=3em]
  ...
  \begin{enumerate}[leftmargin=4em,midpenalty=10]
    \item[(c*)\quad] $a_{i} \geq a^{*}$, \tab (entry time constraint)
  \end{enumerate}
...
The analysis of feasibility conditions in this chapter is very much related to the work~\cite{miculescuPollingsystemsbasedAutonomousVehicle2016}.
%
They study an online situation, in which new vehicles arrive to the system at later times.
%
They also alow changing the crossing order with respect to conflicting lanes, which might require the current planned vehicle trajectories to change (slow down) to anticipate this change.
%
We do not study such an online setting, but we take into account the fact that
lanes are of finite length. \note{[ while revisiting the figures in their paper,
  it seems what they do is very related to my own attempt; I couldn't get
  through their appendix earlier ]}
...
\chapter{Job shop scheduling}\label{sec:job-shop}
