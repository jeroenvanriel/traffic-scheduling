...
\pagestyle{plain}
\raggedbottom
% \usepackage[margin=3.5cm,top=2.8cm,bottom=2.8cm]{geometry} %,top=0.5cm,b/pagottom=1.8cm]{geometry}
\usepackage[a4paper, inner=3.5cm, outer=3.5cm, top=2.8cm, bottom=2.8cm]{geometry}
...
%% full page tables
% \usepackage{pdflscape} % also rotate the page in the pdf file itself
\usepackage{rotating}
...
%% bibliography setup
\usepackage[
  style=numeric-comp, natbib=true,
  doi=false, isbn=false, url=false,
  maxcitenames=1, backend=biber,
]{biblatex}
...
%% set pdf meta-data
\hypersetup{
    pdftitle = {Efficient and Provably Safe Autonomous Intersection Coordination - Combinatorial Learning for Vehicle Motion Planning},
    pdfauthor = {Jeroen van Riel}
}

\title{%
  {\huge\bfseries Efficient and Provably Safe Autonomous Intersection Coordination}\\[1em]
  {\Large\bfseries Combinatorial Learning for\\[-0.3em] 
  Vehicle Motion Planning}
}

\author{%
  \Large Jeroen van Riel \\[12em]
  {\normalsize\itshape Combined Master's Thesis}\\[0.2em]
  \normalsize
  Industrial and Applied Mathematics\\
  \normalsize
  Computer Science and Engineering\\[2em]
  {\normalsize\itshape Supervisors}\\[0.2em]
  \normalsize Marko Boon\\
  \normalsize Stella Kapodistria\\
  \normalsize Mykola Pechenizkiy\\
  \normalsize Danil Provodin\\[12em]
  \normalsize October 2025\\[0.1em]
...
The growing adoption of autonomous vehicles motivates the need for systems that
coordinate joint motion across traffic networks, aiming to reduce travel time,
fuel consumption, and improve comfort. We study this coordination problem under
ideal conditions, assuming perfect communication and a centralized controller
that prescribes precise vehicle trajectories. Focusing on intersection
management, we identify the determination of optimal crossing orders as a key
combinatorial challenge. Formulating delay minimization as a scheduling problem,
we develop an integer programming model and introduce two types of cutting
planes that significantly accelerate solution time for single-intersection
cases. As exact optimization scales poorly, we investigate step-by-step
scheduling as a basis for fast heuristics. For a single intersection, a simple
one-parameter threshold policy achieves less than 2\% and 10\% optimality gaps
for two and three crossing routes, respectively, with up to 60 vehicles per
route. Neural network policies trained via imitation and reinforcement learning
offer limited additional benefit. Finally, we outline challenges in extending
the framework to networks of intersections, particularly in modeling finite lane
capacities, and present preliminary insights into how finite lane capacity
defines the space of feasible crossing time schedules.
 
% \chapter*{Preface}

% The idea for this project is somewhat rooted in the final project of my
% bachelor. Marko, who also supervised me then, asked one question that has stuck
% with me: how to model horizontal queues of vehicles?
% %
% On top of this, I became fascinated by \emph{green waves}, which is a very nice
% idea, but somehow very difficult to achieve in practice.
% %
% Some of the main difficulties lies in the fact that human driving behavior is
% unpredictable and small disturbances tend to propagate and cause all kinds of
% chaotic behavior---congestion and traffic jams.

% \emph{``If you can't win the game, change the rules''.} The situation gets a lot
% more manageable when we assume that all vehicles are autonomous, so human driver
% behavior is no issue anymore.
% %
% In such sitatuion, we can in principle manage congestion far more effectively,
% by having vehicles communicate with each other and optimize their movement.
% %
% Of course, there are many technical problems that we need to solve before such a
% system would work in practice, robust wireless communication being a very
% obvious one.

% When I started this project, I did not really have concrete research questions.
% Instead, there were two topics that I wanted to explore further.
% %
% The first topic, optimal control, has been on my wishlist for quite some time.
% In some sense, optimal control is involved in a lot of tangeable technology,
% like robots, autonomous vehicles---things most people immediately associate
% technical universities with.
% %
% My combined program did not leave much room to go shopping for courses at the
% mechanical engineering department, but I took a course on stochastic decision
% making, where we discussed the modern descendant---reinforcement learning.
% %
% These success stories of super-human level game playing, which were still
% relatively recent at the time, also captivated my imigination.

% The book of Daniel Liberzon was very helpful in shaping some idea of the issues
% that come with optimization over functions. I very much recommend this book as a
% first introduction. The first few chapters trace the historical development of
% variational calculus, which almost makes you feel you are reading a novel.

% The second topic is about using machine learning to speed
% up solvers for combinatorial optimization.
% %
% I was following a course on integer programming when I read some papers
% where they trained neural networks to take certain decisions of the solver,
% e.g., branching variable, cutting plane selection.
% %
% My curiosity was further piqued when I also heard some skepticism about these
% methods.

% In hindsight, you could say that the vehicle coordination problem has been the
% perfect ``playground'' to pursue both these directions.
% %
% I enjoyed an enormous amount of freedom in shaping this project. Admittedly,
% this has also come at some cost; I recognize that I have not been able to
% explore some topics in the depth that I had originally anticipated.

% I remember that I had did not understand why it was so hard to find
% reinforcement learning methods with \emph{hard constraints}, which is what I
% needed to prevent collisions---always.
% %
% I guess it is fair to say that the project has been about dealing with hard
% constraints, but mainly in a model-based manner.
% %
% The same question I had back then is still alive, but has crystallized a little
% bit more: what does model-free reinforcement learning with hard constraints look
% like; how can we use samples to estimate the safe/feasible part of the state
% space?

% Looking back on how the direction of the project has changed over time, there
% has been a clear trend of removing more and more uncertainty from the model, to
% the point where everything was completely deterministic---which seems strange
% for a project with supervisors working in stochastics and operations research.
% %
% Interestingly, the only remaining non-determinism is found in the stochastic
% scheduling policies, which we have discused a number of times.
% %
% At first, I was a little bit unsure about the approach myself, but over time, I
% realized that using stochastic policies is just some way to relax the solution
% space. Similarly, some of the procedures I found in the literature for
% transforming a stochastic policy into a deterministic one are similar to
% rounding in integer programming.

% I would like to thank all my supervisors, for their guidance, motivation and
% patience.

% Marko, thanks for your confidence in me, which I like to believe has caused me
% to gain confidence of my own ideas; I am starting to learn how to tame my inner
% critic.
% %
% Thanks again for involving me in your traffic visualization project. There is
% now finally time to look at the bug you reported; your email has been waiting in
% my inbox for quite some time now.

% Danil, thanks for choosing to work with me. I ended up solliciting with you,
% because you worked on constrained Markov decision processes. At the time, I was
% under the impression that area of literature provided ready-to-use methods to
% solve general constrained decision making processes.
% %
% It turned out this issue was far from solved, and at the time of writing, this
% issue is gaining popularity as \emph{safe reinforcment learning.}
% %
% Since my project only touches upon the issues involved with constraints, I hope
% you enjoyed the alternative course we eventually took.

% Stella, given the fact that supervision is generally fairly regulated within
% your group, it is an honor that you decided to get on board with my project.
% %
% Thanks for seeing things in perspective when I felt stuck.
% %
% Your colorful tips about writing and style have changed my thoughts about what
% makes written communication successful.

% Mykola, although we did not have many interactions, you asked valuable questions
% that have forced me to think about how this project fits in the existing
% literature. To be fair, I think I still find this one of the most difficult
% tasks. I believe you are in a better position to assess this, so I am very
% interested in your unfiltered view on this project.

% Luuc, thanks for reading some earlier drafts of this document and for your help
% with understanding the decomposition proof.

% Thanks to Erjen Lefeber, who helped me gain some confidence that I have not been
% reinventing the wheel while working on the analysis of the capacitated lane
% model.

% Next to my study, I have had the opportunity to keep working in the event
% industry, which has tought me some of the most important lessons in life so far.
...
% \vspace{2\baselineskip} % some vertical space before the signature

% \begin{flushright}
% \textit{Jeroen van Riel}\\[0.5em]
% October 2025
% \end{flushright}
...
\addtocontents{toc}{\protect\renewcommand\cftbeforechapskip{2em}}
...
vehicles and between vehicles and infrastructure, to coordinate their behaviors such
...
speeds, thereby minimizing the need for complete stops at the intersection~\cite{dresnerMultiagentApproachAutonomous2008,khayatianSurveyIntersectionManagement2020,guoUrbanTrafficSignal2019}.
...
junction directly influences neighboring ones. Coordinated strategies can be
extended across the entire network, enabling the optimization of vehicle
trajectories across multiple intersections.
...
technological innovation~\cite{padmajaExplorationIssuesChallenges2023,yangAutonomousDrivingV2X2022} as well as policy development,
involving questions of how to adopt autonomous driving for safe~\cite{hilaryMakingAutomatedVehicles2022},
accessible~\cite{fatimaEquityIssuesAssociated2024} and trustworthy~\cite{naisehTrustRiskPerception2025}
...
\begin{itemize}
  ...
  \item \emph{Performance objectives and user preferences.} From the perspective of
        ...
        central---for example, one line of work~\cite{reyOnlineIncentiveCompatibleMechanisms2020,sayinInformationDrivenAutonomousIntersection2019} investigates
        auction-based systems where users bid for priority service at
        intersections.
  
  \item \emph{Decentralized control and communication.} A key design issue is
        how to organize communication and control in such systems.
        %
        Centralized coordination with full observability is nearly impossible in
        practice due to noisy sensors, communication delays~\cite{wangAssessingImpactCommunication2024},
        inaccurate information~\cite{vitaleAutonomousIntersectionCrossing2022}, and scalability challenges. The majority
        of proposals assume some sort of decentralized architecture~\cite{yangAutonomousDrivingV2X2022},
        where low-level control is handled by individual vehicles while a
        central entity focuses on higher-level coordination.
  ...
  \item \emph{Graceful adoption of autonomous driving.} Fully autonomous traffic without
        ...
        %
        Interestingly, even when autonomous vehicles make up only a small
        fraction of the total traffic, they can yield substantial efficiency
        gains by influencing human driver
        behavior~\cite{wangLearningControlCoordinate2025}, thereby reducing
        congestion.

  \item \emph{Robustness to uncertainty.}
        %
        Uncertainty and partial observability are fundamental aspects of all
        coordination formulations.
        %
...
While acknowledging the breadth of modeling choices in the context of autonomous
intersection coordination, we focus on a simplified formulation that exposes the
fundamental challenges of large-scale coordination, and study an idealized
traffic model that captures only the essential components.
...
This section highlights some research directions that are most
relevant to our formulation of safe and efficient motion planning.
%
We refer to the survey
papers~\cite{marianiCoordinationAutonomousVehicles2022,khayatianSurveyIntersectionManagement2020}
for a more comprehensive overview of the literature.
...
A common approach is \emph{direct
  transcription}---discretizing
time to obtain a finite-dimensional program~\cite{kellyIntroductionTrajectoryOptimization2017}---for which hard constraint
satisfaction is straightforward to achieve. However, this method becomes
computationally expensive as the number of vehicles, intersections, and time
steps grows.
%
When vehicle dynamics are simple and explicitly known, analytical solutions can
sometimes be derived~\cite{hartlSurveyMaximumPrinciples1995}, but enforcing hard
safety constraints makes this notoriously difficult. For more complex or
uncertain dynamics, learning-based approaches can be used to approximate the
dynamics or control policies, but ensuring safety and interpretability remains a
major challenge.

\paragraph{Reservation-based coordination.}
% adjacent: auction, bidding, fairness

Instead of emphasizing the inherent optimization problem,
a large body of work treats intersection management as a \emph{reservation problem}:
vehicles communicate with a central controller to book time-space resources in
the conflict area. The seminal \emph{Autonomous Intersection Management} (AIM)
framework~\cite{dresnerMultiagentTrafficManagement2004,dresnerMultiagentApproachAutonomous2008}
pioneered this idea. Vehicles submit crossing requests that are granted or
rejected depending on previously accepted reservations, ensuring collision-free
use of the intersection. While this paradigm provides strong safety guarantees,
it offers limited control over the detailed vehicle trajectories and can be
conservative in throughput.

Numerous refinements have sought to improve the efficiency of the reservation
process. Examples include more accurate estimation of arrival
times~\cite{auMotionPlanningAlgorithms2010}, prioritization or weighting of
reservation requests~\cite{huangAssessingMobilityEnvironmental2012}, and
time-aware extensions that explicitly account for communication
delays~\cite{khayatianCrossroadsTimeawareApproach2020}. Other studies extend AIM
to \emph{networks of intersections}, coupling reservation decisions with dynamic
routing~\cite{hausknechtAutonomousIntersectionManagement2011}, or employ more
precise geometric conflict-detection schemes beyond the original cell
discretization~\cite{levinConflictpointFormulationIntersection2017,liTemporalspatialDimensionExtensionbased2019}.

Despite these advances, reservation-based systems remain largely reactive:
vehicles request access individually, and the system does not explicitly
optimize global schedules. This limitation has motivated a complementary line of
research focused on \emph{direct optimization of crossing orders}.

\paragraph{Crossing order optimization.}

% \emph{Crossing order optimization.}
Trajectory optimization is inherently non-convex because collision-avoidance
constraints impose an implicit \emph{vehicle crossing order decision} at
intersections. Rather than relying on fixed traffic signal phases, these
ordering decisions can, in principle, be optimized.
%
...
Instead of handling sequential requests, approaches focused on crossing order
treat intersection coordination as a scheduling problem, in which a central
manager determines the sequence and crossing times of individual vehicles or
platoons---groups of consecutive vehicles.

Many works in this direction address the \emph{online} variant, where
vehicles arrive stochastically.
%
In this setting, the goal is to find a \emph{scheduling policy} that updates the
schedule every time a new arrival happens.
%
This has been achieved by solving a mixed-integer or nonlinear optimization
problem in a rolling horizon
fashion~\cite{levinConflictpointFormulationIntersection2017,liTemporalspatialDimensionExtensionbased2019}.
%
To reduce combinatorial complexity, it is convenient to determine the crossing
time of all vehicles in a platoon at
once~\cite{tallapragadaHierarchicaldistributedOptimizedCoordination2017,tallapragadaDistributedControlVehicle2017}.
%
Guaranteeing safety of trajectories
after schedule updates is a central challenge in the online setting.
%
This has been rigorously analyzed for a rescheduling scheme that was inspired by
viewing the intersection as a polling
system~\cite{miculescuPollingsystemsbasedAutonomousVehicle2016} and
corresponding trajectories have been shown to have closed form
expressions~\cite{timmermanPlatoonFormingAlgorithms2021}.

% joshi
Crossing order decisions become particularly interesting when vehicles with
heterogeneous dimensions and dynamics are considered~\cite{joshiTrajectoriesPlatoonformingAlgorithm2025}. For example, since
trucks accelerate more slowly than passenger vehicles, it is sensible to treat
them separately to avoid unnecessary interference between the two groups.
...
While heuristics have been proposed in the literature,
...
relatively scarce.
%
One strategy is to decompose the problem into an upper-level crossing time
scheduling problem and lower-level problems for generating continuous-time
trajectories.
%
Hult et al.~\cite{hultApproximateSolutionOptimal2015} minimize a quadratic objective over vehicle velocities
through a two-stage process: the upper level optimizes crossing times based on
approximate cost functions, and the lower level computes corresponding
trajectories via quadratic programming. This approach substantially reduces
runtime but is limited to single-intersection settings. The bilevel formulation
is also seen in a procedure where the trajectory optimization and schedule
optimization are performed in alternating
fashion~\cite{zhaoBilevelProgrammingModel2021}.
...
% \begin{tabularx}{1.8\textheight}{@{} l L{0.48} L{0.40} L{0.3} L{0.6} @{}}
% \toprule
% \textbf{Reference} &
% \textbf{Key Paradigm} &
...
% \midrule
% \citet{dresnerMultiagentApproachAutonomous2008} &
% Reservation of grid cells for trajectory in intersection &
% Multiagent online &
% Delay minimization &
% Simplified vehicle model; global heuristic \\

% \citet{levinConflictpointFormulationIntersection2017} &
% Mixed-integer linear programming &
% Centralized; rolling horizon optimization &
% Travel delay &
% Optimization of intersection entry speed  \\
% \citet{tallapragadaHierarchicaldistributedOptimizedCoordination2017,tallapragadaDistributedControlVehicle2017} &
% Hierarchical control &
% Multiagent; online feedback control  &
% Travel delay and fuel consumption &
% Robust to communication failures \\

% \citet{miculescuPollingsystemsbasedAutonomousVehicle2016} &
...
% \citet{timmermanPlatoonFormingAlgorithms2021} &
...
% \end{tabularx}
% \end{adjustbox}
% \captionof{table}{Selection of works focusing on motion planning for autonomous
%   intersection coordination.}
% \end{landscape}

% % Second table
% \begin{adjustbox}{scale=0.9,center}
% \begin{tabularx}{1.4\textheight}{@{} l L{0.5} L{1.5} @{}}
% \toprule
% \textbf{Reference} &
% \textbf{Problem Focus} &
...
% \end{tabularx}
% \end{adjustbox}
% \captionof{table}{Some key references on machine learning for combinatorial optimization.}%
% \label{tab:ml-co-literature}

\section{Project overview}

We believe that one of the key computational bottlenecks in autonomous
intersection coordination across multiple networks lies in the \emph{combinatorial crossing order}
problem. Rather than pursuing a fully joint optimization of continuous
trajectories and discrete crossing orders---which is theoretically elegant but
computationally prohibitive---we try to deliberately decouple these aspects.

By focusing on the combinatorial component, we can exploit recurring structural
patterns that emerge in practical traffic systems.
%
In particular, recent advances in machine learning for combinatorial
optimization~\cite{bengioMachineLearningCombinatorial2020,mazyavkinaReinforcementLearningCombinatorial2020,gasseExactCombinatorialOptimization2019,koolAttentionLearnSolve2019,zhangLearningDispatchJob2020}
demonstrate that algorithms can be trained to recognize patterns in families of
problem instances, yielding fast, data-driven heuristics.
%
...
structure. By training on representative instances, we can potentially discover
coordination strategies specifically tuned to current conditions.

Motivated by the recent successes in this field---which we call
\emph{combinatorial learning}---we aim to demonstrate how machine learning
algorithms can be used to derive heuristic policies for combinatorial problems
that arise when coordinating autonomous vehicles. In this sense, our
contribution is both methodological and conceptual: we show how modern
learning-based methods can be integrated into the classical trajectory
optimization setting, preserving interpretability and safety while improving
scalability.

Our work has been organized around the following research questions:

\begin{enumerate}[label=\textbf{RQ\arabic*}:, leftmargin=3em, rightmargin=4em]
  \item Under which assumptions can the combinatorial aspect of the
        problem---determining the crossing order---be considered in isolation?
        (decomposing)

  \item How well do existing methods solve the crossing order problem?
        (complexity)

  \item How to leverage the recent successes in combinatorial learning to solve
        the crossing order problem arising in safe motion planning? (learning)

  \item How to extend the previous questions to networks with multiple connected
        intersections? (scaling)
\end{enumerate}
...
\pagebreak
The contributions of our project can be summarized into corresponding
categories:

\paragraph{RQ1: Isolating the crossing time scheduling problem.}

After we establish the precise single intersection model in~\Cref{chap:single},
our first contribution is to show that, under certain conditions, the joint
...
\paragraph{RQ2: Cutting planes for crossing time scheduling.}

We illustrate the classical method of manually trying to develop good algorithms
in~\Cref{chap:crossing-time-scheduling}.
%
The crossing time scheduling problem is formulated as mixed-integer linear
program. Explicit knowledge about the problem's structure leads to the
definition of cutting planes, which speeds up the MILP solving significantly.

\paragraph{RQ3: Combinatorial learning for crossing time scheduling.}

Our most important contribution is an illustration of how machine learning
techniques can be applied to solve scheduling problems in practice.
%
After a general introduction of the combinatorial learning paradigm at the start
of~\Cref{chap:single-learning}, we formulate the scheduling problem as a
sequential decision-making process using the Markov decision process framework.
%
We define two parameterized classes of algorithms: a single-parameter class,
which is optimized by a simple grid search, and a neural parameterization, that
we optimize using imitation learning, using optimal solutions as expert
demonstration, and reinforcement learning.
%
Both approaches lead to heuristics that are relatively cheap to train.


\paragraph{RQ4: Preliminaries for coordination across multiple intersections.}

We show a way to extend isolated intersection coordination to multi-intersection
networks in~\Cref{chap:network-learning}. We show under which assumptions this
problem can still be formulated as a scheduling problem, which turns out to be
an extension of the classical job shop scheduling problem.
%
In this setting, feasibility of trajectories deserves special attention, which
is explored in~\Cref{chap:capacitated-lanes}.

% \subsection{Opportunities of combinatorial learning}

% % \paragraph{Learning from problem structure.}
% The coordination problems discussed above exhibit regularities that can
% potentially be exploited algorithmically. Traditional approaches rely on
% manually designed heuristics to capture such structure, but recent advances in
% machine learning for combinatorial optimization suggest an alternative: treating
% problem instances as data and learning to recognize patterns associated with
% good solutions.

% A key insight from this literature---to which we will refer as
% \emph{combinatorial learning}---is that practical applications rarely require
% optimal solutions for all conceivable instances. Instead, problem instances are
% typically drawn from a limited distribution with predictable
% characteristics—such as recurring traffic patterns, common vehicle routes, or
% structured networks. By focusing on expected performance over these
% distributions, rather than worst-case guarantees, heuristics can be learned that
...
Within the controlled environment sketched in the introduction, this chapter will
...
\begin{figure}
  ...
  \caption{Stylized model of a single intersection with two conflicting flows of
    vehicles that are not allowed to turn at the intersection, which means that
    we can assume that vehicles cross the intersection in straight
    lines.}\label{fig:intersection-nice}
\end{figure}

\paragraph{Trajectory optimization.}
The task of safely and efficiently coordinating the motion of autonomous
vehicles through our single-intersection model, can be formulated mathematically
as \emph{trajectory optimization}, which is precisely defined in
Section~\ref{sec:motion-planning}.
% direct transcription
Perhaps the most natural way of solving this kind of problems depends on
...
intractable relatively quickly, when considering larger numbers of vehicles or
making the time discretization finer.
%
...
crossing order as a separate stage, and compute the corresponding vehicle
...
Mathematicians naturally seek the most general possible
...
\end{itemize}
%
\emph{Under these assumptions, the decomposition is proper}. Consequently, the
...
Let us now proceed to consider arbitrary numbers of vehicles. We introduce
the following notation to identify routes and the vehicles associated with them.
...
\begin{align}\label{eq:safe-crossing}
  (x_{i}, x_{j}) \notin \mathcal{E}^{2} 
\end{align}
for each conflict $\{i,j\} \in \mathcal{D}$ guarantees the absence of
...
dynamical behavior of the system. In other words, we describe how the positions
of vehicles are allowed to change as a function of time.
%
...
%
Furthermore, note that the occurrence of $(\cdot)^2$ is generally preferred over
...
\begin{description}[leftmargin=\parindent,labelindent=\parindent,labelwidth=\widthof{\bfseries collision-free }]
  ...
  \item[efficient] $\rightarrow$ $x_i$ are optimal w.r.t.\ some criterion of
...
\begin{figure}
  ...
  \caption{Example of optimal trajectories minimizing $J_{\alpha,\beta}$ (solid)
    ...
    distinct colors. The interval of intersection-occupying positions
...
\end{subequations}
%
The set of times in these definition are closed, because $x_{i}$ is continuous
by assumption, but they may be empty. We use the convention that
``$\min \varnothing = \infty$'', such that $\tau(x_{i}) = \infty$ whenever
$x_{i}$ never reaches the intersection, and 
$\xi(x_{i}) = \infty$ whenever it never reaches the end of the intersection.
Furthermore, we use the convention that ``$(\infty, \infty) = \varnothing$''.
  %
...
%
Similarly, observe that the collision-avoidance constraints~\eqref{eq:T3}
% \begin{align*}
...
Let us introduce some notation for collections of parameters and variables
pertaining to a single route.
%
...
\paragraph{Lower-level control problem.}
Consider some route $r \in \mathcal{R}$ with initial conditions $s_{r}$ and
suppose we are given some fixed occupancy time slots, given by
$y_{r} := \{y_{i} : i \in \mathcal{N}_{r}\}$ and
$z_{r} := \{z_{i} : i \in \mathcal{N}_{r}\}$, then we define the lower-level \emph{control problem}
%
...
\paragraph{Upper-level scheduling problem.}
...
which yields the variant~\eqref{eq:T*} shown below. With this technical assumption, we
briefly sketch how validity of the decomposition can be established rigorously.
\begin{alignat}{3}\label{eq:T*}
...
The decomposition is just a reformulation of the problem; it does not
immediately provide a better way of solving the original problem.
%
...
easily characterizable, e.g., as a system of inequalities. In other words, the
upper and lower problems are \emph{tightly coupled} by optimality and feasibility.

Nevertheless, the reformulation provides a good conceptual basis for
...
not aim to tackle the problem in its full generality. Instead, we make some mild
...
\begin{define}%[Delay criterion]
  % We introduce the following simple trajectory cost criterion.
  Let the \emph{delay criterion}
  be defined $J_d(x_i) = \tau(x_i) - a_i$.
  % To simplify notation in the
...
spend a few words on explaining our notation for specifying instances
of this problem.
%
...
In what follows, we will use a simple way of visualizing
...
single row, such that every block starts at the schedule time $y_i$, see
Figure~\ref{fig:solution-example}. Therefore, every block represents the
crossing time slot that is allocated to the corresponding vehicle. We 
add arrows of length $\delta = \sigma - \rho$ to illustrate the necessary switch-over time
between consecutive time slots for different routes.

\def\vizfigscale{0.90}
\begin{figure}[h]
  \centering
  \includegraphics[scale=\vizfigscale]{figures/single/instance}
...
thesis of Robert Hult~\cite[Chapter 3]{hultOptimizationBasedCoordination2019}.
%
...
Problems like crossing time scheduling~\eqref{eq:C} have been studied extensively, so we
first discuss an established solution method.
%
...
%
One of the most prominent distinction among algorithms lies in whether they
guarantee to find an optimal solution or not.
%
...
combinatorial problems, in which the discrete decisions are encoded as integers.
More specifically, in~\Cref{sec:branch-and-bound}, we show how~\eqref{eq:C} can be
formulated as a \emph{mixed-integer linear
  program}~\cite{confortiIntegerProgramming2014}, which can be solved in a
...
formulate two problem-specific cutting planes, yielding an efficient
...
optimality guarantee in favor of speed. In other words, we want a good
solution---fast!
...
each conflict $\{i,j\} \in \mathcal{D}$, this constraint essentially encodes
...
Gurobi~\cite{gurobi} (commercial), employ specialized internal heuristics and
...
\subsection{Optimal substructure: platoon preservation}\label{sec:platoons}
...
%
The branch-and-bound framework with cutting planes is colloquially referred to as
\emph{branch-and-cut}.
...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.9]{figures/single/123.pdf}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.9]{figures/single/platoons-color.pdf}
  \caption{Different ways to split platoon A (in blue), regarding Example~\ref{example3},
    assuming equal earliest crossing times $a_{A} = a_{B}$ with $n_{A} = 3$ and $n_B = 4$.}
  \label{fig:example3}
\end{figure}
...
\pagebreak[4]
\subsection{Cutting planes}\label{sec:cuts}
Based on the above analysis of optimal substructure, we will define two types
...
\end{align}
We refer to these constraints as \emph{conjunctive cutting planes}.
...
implies that whenever we have $i \rightarrow k$, for some $k \in \mathcal{N}$ such that
$r(i) \neq r(k)$, then we must have $j \rightarrow k$, which can be enforced by adding the
constraints
\begin{subequations}
\begin{align}
  b_{ij} + (1 - \gamma_{ik}) + \gamma_{jk} \leq 2 . \tag{disj.cut}
  % b_{ij} + \gamma_{ik} + (1 - \gamma_{jk}) \leq 2 . \tag{disj.cut.2}
\end{align}
\end{subequations}
We refer to these constraints as \emph{disjunctive cutting planes}.
...
We conclude this section with an evaluation of the running time of the integer
programming approach and of the additional benefit of the proposed cutting
planes.
%
Instead of only reporting the required runtime to obtain an optimal solution, we
use a slightly more fine-grained metric that also shows how fast the solver
progresses towards an optimal solution.
%
At time $t \geq 0$ since the start of the algorithm, we know the objective
$\mathrm{obj}(t)$ of the best solution found so far and the current best lower
bound $\mathrm{bnd}(t)$ on the optimal objective.
%
This leads to the following definition of the \emph{intermediate optimality gap}
\begin{align}
  \mathrm{gap}(t) = \frac{\mathrm{obj}(t) - \mathrm{bnd}(t)}{\mathrm{obj}(t)} .
\end{align}

We compare the relative performance of integer programming without any
additional cutting planes (\emph{plain}), with each of the two types of cutting
planes defined above (\emph{conj} and \emph{disj}) and with both cutting planes
together (\emph{conj}+\emph{disj}).

Each of these configurations is executed on some problem instances with fixed
$\rho=1.2$ and $\sigma=1.7$ and randomly sampled earliest crossing times $a_i$.
%
We hypothesize that the difficulty of an instance is mainly determined by the
total number of vehicles it considers, so we consider two different fixed sizes
of problems: \emph{small} instances with $R=2$ routes and $n_r = 20$ vehicles
per route, and \emph{large} instances with $R=3$ routes and $n_r = 20$ vehicles
per route.
%
To keep the total benchmark running time reasonable, we limit the runtime of
each solver execution to a minute, which also makes sense in the context
real-time optimization.
%
The results are shown in Figure~\ref{fig:running-time}.
It is clear that both types of cutting planes provide some runtime improvement, but they are best when combined.

\begin{figure}
  \centering
  \begin{subfigure}[b]{1.0\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../single/results/figures/milp-bench-small.pdf}
    \caption{Small instances with 40 vehicles in total.}%
    \label{fig:a}
  \end{subfigure}
  \par\vspace{2em}
  \begin{subfigure}[b]{1.0\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../single/results/figures/milp-bench-large.pdf}
    \caption{Large instances with 60 vehicles in total.}%
    \label{fig:b}
  \end{subfigure}
  \caption{Progress of integer programming in terms of the intermediate gap
    over time.
    %
    We keep track of the gap by using the callback interface of Gurobi. We
    report the gap every 0.5 second or when the gap changes more than $1\%$, to
    limit the overhead of the callback.
    %
    For this benchmark, we used the Gurobi solver version 11.0.2 on a system
    with a 13th Gen Intel i5-13600K CPU and 32GiB of RAM.}%
  \label{fig:running-time}
\end{figure}

% \begin{figure}
%   \centering
%   \includegraphics[width=0.95\textwidth]{../single/figures/milp-benchmark-styling.pdf}
%   \caption{Average run time of branch-and-cut as a function of the number of
%     arriving vehicles per route.}%
...
Observe that the binary variables $\gamma_{ij}$ encode the crossing order.
% 
We show that the crossing order is all we need to know to encode an optimal
solution $y$.
%
Specifically, after fixing binary variables $\gamma_{ij}$, we obtain a linear
program, which can be shown to have a unique solution whenever it is feasible.
%
...
\begin{figure}[b]
  ...
  \caption{Disjunctive graph for an instance with $R=2$ routes, and $N=7$
    vehicles in total, with $n_{1} = 3$ on the first route and $n_{2} = 4$ on
    the second route. Dummy nodes are not shown. The horizontal arrows represent
    the conjunctive arcs, the other arrows represent some partial choice
    $\mathcal{O}$; the remaining dashed lines depict pairs of disjunctive
    arcs that are not chosen.}\label{fig:disjunctive-graph-intro}
\end{figure}
...
%
This way, we essentially reduce the infinite space of candidate schedules to
...
order can be constructed using a simple procedure in which the routes are
visited according to the route order. We can think of the vehicles
...
Figure~\ref{fig:disjunctive-graphs} illustrates the above discussion by showing the three related
...
The discussion above is now summarized in the following result.
...
Historically, a lot of effort has gone into developing \emph{local search
  heuristics} for combinatorial optimization problems.
%
It is also not difficult to come up with some local search procedure
for~\eqref{eq:C}, which we illustrate in~Appendix~\ref{sec:local_search}.
%
I would like to take this opportunity to warn the reader for the abuse of
metaphor in the local search
literature~\cite{sorensenMetaheuristicsMetaphorExposed2015}.


\chapter{Learning to schedule}\label{chap:single-learning}

Recent years have seen increased interest for using machine learning (ML)
techniques to tackle combinatorial optimization (CO) problems.
%
The main underlying motivation is that combinatorial problems contain structure
that is difficult to formalize precisely, and the hope is that learning
algorithms can help with discovering, encoding and exploiting these structures
algorithmically.

This chapter begins with a very brief survey of the main ideas and methods in
this relatively new area of research.
%
The rest of this chapter details our own version of this methodology for solving
the crossing time problem introduced in~\Cref{chap:single}. Our main idea is to
encode a step-by-step scheduling approach as a Markov decision process, which we
then try to solve using tools from the reinforcement learning literature.

\section[Combinatorial learning]{Combinatorial learning}
...
\caption{Some key references on combinatorial learning.}%
\label{tab:ml-co-literature}
\begin{adjustbox}{center}
\scalebox{0.8}{
\begin{tabularx}{1.35\textwidth}{@{} l L{0.5} L{1.5} @{}}
\toprule
\textbf{Reference} &
\textbf{Problem focus} &
\textbf{Contribution / Method} \\
...
\citet{gasseExactCombinatorialOptimization2019} & MILP & Approximate strong branching policy with GNNs in branch-and-bound \\
\citet{tangReinforcementLearningInteger2020} & ILP & Attention-based model for cutting plane selection trained with RL \\
\citet{gravesNeuralTuringMachines2014} & General computation & Neural turing machine as a differentiable model of computation \\
\bottomrule
\end{tabularx}}
\end{adjustbox}
\end{table}

Some key publications on which the following discussion is based are collected in
Table~\ref{tab:ml-co-literature}.
%
The following presentation draws heavily from \cite{bengioMachineLearningCombinatorial2020}.
...
an optimal configuration vector $v \in V$ from a finite set of candidates, such
...
Two main motivations for applying ML for solving CO problems can be identified
in the current literature:

\begin{enumerate}
  \item{Fast approximations.} We might want to replace certain expensive
        computations of an existing algorithms such as branch-and-bound with
        fast ML approximations.
        %
        Please note that the approximate nature of ML models does not
        necessarily impact optimality guarantees of the original algorithm.
        %
        This is clearly illustrated by~\cite{gasseExactCombinatorialOptimization2019}, where the branching decision
        in branch-and-bound is approximated, and the line of work related
        to~\cite{tangReinforcementLearningInteger2020}, where ML models are employed for cutting plane selection.
        %
        In both examples, the algorithm is still guaranteed to obtain an optimal
        solution, as long as the model provides valid branching decisions or
        valid cutting planes.
        %
        Schemes in which ML models are integrated within some existing CO
        algorithm have been referred to as \emph{joint} methods.

  \item{Algorithm discovery.} Instead of trying to improve existing algorithms,
        we could try to employ ML to explore the space of algorithms to find
        new and hopefully better algorithms.
        % 
        This type of approaches can also be understood as constructing a
        solution from scratch or in terms of predicting the optimal solution,
        given the problem instance.
        % 
        The method that we propose in~\Cref{sec:MDP} belongs to this class of
...
it somehow captures model structure, similar to how we as humans develop
intuition about the problem.

In what follows, we will make this notion more precise by presenting the process
of designing algorithms for CO problems as an optimization problem in and of
itself.
%
...
\subsection{Optimizing over algorithms}

% The process of designing an algorithm itself may be considered as an
% optimization problem.
%
Let $\omega = (V, f) \in \mathcal{I}$ denote some instance of a combinatorial problem
and let $\pi \in \Pi$ denote some set of possible algorithms for solving it---we will
make the definition of $\Pi$ more precise later.
%
...
are only dealing with a narrow subset of $\mathcal{I}$, often consisting of very
similar problems.
%
...
\begin{figure}
  ...
  \caption{\emph{Illustration of how problems occuring in practice have a
    ...
    ring road, whereas large parts of Montreal are fairly Manhattan-like.
    Statements like these are clearly cumbersome to formalize precisely, which
    is one of the main motivations for employing techniques from machine
    learning.}\label{fig:problem-distribution}
\end{figure}
...
Python programs, for example.
%
However, real-world code runs on a complex stack of compilers, interpreters, and
other complicating factors like hardware optimizations. Therefore, it might be
more desirable to work with simplified models of computation.

A common modeling choice in the ML for CO literature is to represent algorithms
as policies for Markov decision processes (MDPs). This perspective naturally
connects algorithm optimization to reinforcement learning methods, which many
existing works indeed leverage~\cite{mazyavkinaReinforcementLearningCombinatorial2020,daiLearningCombinatorialOptimization2018,koolAttentionLearnSolve2019}. In what follows, we
show how the MDP framework arises naturally as a model of computation and
explain why it provides a particularly useful abstraction within the contex of
algorithm optimization.

\subsection{Modeling a class of learnable algorithms}
...
state transition sequence of some deterministic finite-state automaton (DFA). Let
...
measuredin terms of the unique final state $s_f^\pi$ by some
cost function $c : \mathcal{S}_f \rightarrow \mathbb{R}$, such that we have
$m(\omega,\pi) = c(s_f^\pi)$.
%
To accommodate this, states may need to carry auxiliary meta-information such as
total number of transitions or bounds on the quality of the solution. Actual
running time is not captured by this simple model formulation, but the number of
transitions can be used as a proxy.
...
crafting a more restricted state space.
...
in the MDP, because $s_f^\pi$ is now a random variable.

\paragraph{Constraint satisfaction with neural networks.}

A major challenge when using neural networks to parameterize policies is
constraint satisfaction, or in other words, making sure valid actions are
selected. For example, solutions produced by neural policies need
to satisfy the constraints of the original combinatorial problem.
%
This issue has led to the development of neural network components whose outputs
satisfy some specific type of constraint, for example: being a permutation of
the input, for which the paper on \emph{pointer
  networks}~\cite{vinyalsPointerNetworks2017a} is a seminal example.

Recent works focus more heavily on using some sort of correction mechanism. For
example, the DC3 framework~\cite{dontiDC3LearningMethod2021} employs two
differentiable processes, completion and correction, to solve any violations of
equality or inequality constraints, respectively. The more recent HardNet
framework~\cite{minHardConstrainedNeuralNetworks2024} uses a closed-form
...
\pagebreak
\paragraph{``Algorithm as MDP policy''.}

We summarize the algorithm optimization paradigm outlined above, by listing the
main components of the MDP and their modeling role:
...
\begin{enumerate}[label=$\bullet$,leftmargin=2.5em,rightmargin=4em,midpenalty=5]
  ...
  \item The \textbf{state transitions} determine how instructions are executed. Stochastic
        state transition can be used for components of an existing algorithm
        that are too complex and expensive to model explicitly.
...
We will now apply combinatorial learning paradigm to the crossing time
problem~\eqref{eq:C}.
%
The most important decision is the design of the state space of the MDP
algorithm model, since this essentially determines the class of algorithms over
which we are optimizing---it is our main way of incorporating prior knowledge
and intuition into the model.
%
...
\begin{figure}[p]
  \centering
\begin{tikzpicture}[arrow/.style={-latex}]
    \node[anchor=south west, inner sep=0] (img) at (0,0)
      {\includegraphics[scale=0.9]{figures/single/episode.pdf}};
    \begin{scope}[x={(img.south east)}, y={(img.north west)}]
    \draw[arrow] 
    (1,0.94) .. controls (1.05,0.86) and (1.05,0.82) .. (1,0.74) 
    node[midway, right] {$\eta_1 = 2$};

    \draw[arrow] 
    (1.0,0.655) .. controls (1.05,0.575) and (1.05,0.535) .. (1,0.455)
    node[midway, right] {$\eta_2 = 1$};

    \draw[arrow] 
    (1.0,0.37) .. controls (1.05,0.29) and (1.05,0.25) .. (1,0.17) 
    node[midway, right] {$\eta_3 = 2$};
      
    \end{scope}
  \end{tikzpicture}
  % \includegraphics[scale=1]{figures/single/episode.pdf}
  \caption{\emph{Visualizing an episode of the MDP.} Every one of the four frames in
    ...
    %
    Hence, the first and last frame visualize the initial state and a complete
    schedule, respectively, in the same way as before. The frames in between
    show partial schedule states and the arrows indicate the route that is
    scheduled by each action. The solid boxes in the top row of each frame show
    the current partial schedule, the other rows show the unscheduled vehicles
    of the other routes. Note that we draw the ``switch'' arrow only for the
    routes other than the current and we draw them in the final complete
...
\clearpage
\subsection{Combinatorial learning objective}
...
A common starting point is to let $\Pi$ be all stochastic policies
$\pi : \mathcal{S} \rightarrow \Delta(\mathcal{A})$.
%
Even more general classes of policies could be considered, by allowing the next
action to depend on the entire history of states and actions encountered so far.
However, it can be shown that this does not produce more powerful policies, in
the sense that there always exists an optimal policy that is Markov, i.e, is
only a function of the current state.
%
Since the state space of $\mathcal{M}_\mathrm{constr}$ decomposes into finite
subspaces as sketched above, it can be shown that the following class of
policies is general enough, in the sense that it contains optimal policies.
...
\section{Efficient policy parameterization}

With the definition of the MDP, the way we parameterize the class of admissible
policies is the second way of incorporating prior knowledge into the learning
pipeline.
%
Before we present two types of concrete policy parameterizations, we first discuss some redundancy in the state space of the initial definition of $\mathcal{M}_\mathrm{constr}$.


\subsection{State abstraction function}
After a closer look at $\mathcal{M}_\mathrm{constr}$, we identify some sort of
symmetry in the state space.
%
Such redundancy can potentially hamper our ability to find a good policy.
%
Instead of redefining the MDP, it will be convenient to address this redundancy
through a \emph{state abstraction function}, which maps states that are, in some sense,
similar to a single canonical state.
%
More specifically, it can be shown that the optimal completion of a partial
schedule does not depend on the vehicles that have already been scheduled, see
for example~\Cref{fig:abstract}.
%
...
\begin{enumerate}
  ...
  \item It is not difficult to see that the optimal completion of a partial
        ...
        %
        Hence, we fix a unique reference time for the partial schedule and
        subtract the reference time from the lower bound of each unscheduled
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{../single/figures/abstract.pdf}
  \caption{Two states of $\mathcal{M}_\mathrm{constr}$ that only differ by a
    fixed shift of time. They are equivalent with respect to optimal
    completions: it can be shown that any sequence of actions that optimally
    completes the upper partial schedule would also optimally complete the lower
    one.}%
  \label{fig:abstract}
\end{figure}
...
$n_r(s)$ and $k_r(s)$ denote the total number of vehicles and the number of
...
We define the \emph{abstract state mapping} $\phi : \mathcal{S} \rightarrow \mathcal{S}$ by showing
how to construct the abstract state $\phi(s)$.
%
...
\end{align}
%
which can be interpreted as some kind of \emph{current time} of the
partial schedule.
...
\begin{theorem}
  ...
  there exists some function $f : \mathcal{S} \rightarrow \mathcal{A}$ such that
  $\pi^* = f \circ \phi$.
\end{theorem}
\begin{proof}[Proof (sketch)]
  ...
  Definition 3 in~\cite{liUnifiedTheoryState2006}, then Theorem 3 of the same paper
...
Next, we provide two suggestions for parameterizing this function $f$.

\subsection{Guaranteed platoon preservation with threshold heuristics}\label{sec:threshold}
...
The following property is straightforward to verify from this definition.
...
$\{\tau_k \} \subset [\tau_{\min}, \tau_{\max}]$ and evaluating
\begin{align*}
  \tau^* := \argmax_{\tau \in \{\tau_k\}} \sum_{\omega \in D_\textrm{train}} R(\eta(\omega, \pi_{\tau})) .
\end{align*}
%
...
\subsection{More freedom with neural parameterizations}\label{sec:neural-param}

Given the recent successes of combinatorial learning, as surveyed at the
beginning of this chapter, it seems a natural to parameterize $f$ using a neural
network model.
%
This frees us from the task of manually trying to develop a good
parameterization of $f$ and opens up the possibilities of finding structure in
problem instances. By using a neural parameterization, we weaken the amount of
prior we put in the policy, in some sense.

\paragraph{Neural architecture.}

Recall that each abstract state is determined by the set of route arrival
sequences
\begin{align}
  \bar{a}_r := \{ \bar{a}_{rk} : 1 \leq k \leq \bar{n}_r \} ,
\end{align}
for each $r \in \mathcal{R}$, where $\bar{n}_r$ denotes the number of
unscheduled vehicles on this route.
%
A straightforward way would be to just take the flattened vector $\bar{a}$ as
input and consider a fully connect neural network
$f^{\mathrm{full}}_\theta(\bar{a})$.
%
Alternatively, we could for example encode each sequence $\bar{a}_r$ using a
recurrent neural network $f^{\mathrm{RNN}}_{r,\theta}$ to produce a \emph{route
  embedding} $f^{\mathrm{RNN}}_{r,\theta}(\bar{a}_r)$.
%
The concatenation of these route embeddings is then fed through a fully connected
layer $f^{\mathrm{full}}_\theta$.
%
In both cases, we need a final softmax layer to produce logits that encode a
probability distribution over actions.


\paragraph{Gradient-based parameter optimization.}
Regardless of the precise architecture, a neural parameterization has generally
many parameters, which we will denote as the vector $\theta$. Obviously, a
simple grid search is no longer tractable.
%
In order to use gradient-based parameter optimization, we will explicitly
consider stochastic policies. This ensures that the gradient of the expected
return in~\eqref{eq:MDP} with respect to $\theta$ is properly defined.
%
We call the stochastic policy the \emph{training policy} $\pi_\theta$.
%
Note that eventually want a deterministic \emph{target policy} $\pi_\mathrm{d}$.
This means that we need to specify some way to make the training policy deterministic again.
One way would be to use the following simple greedy action selection.
%
Let $\pi = f \circ \phi : \mathcal{S} \rightarrow \Delta(A)$ be some stochastic
...
When we have access to some \emph{expert policy}, which we know performs well on
the task at hand, a natural strategy is to try to imitate its behavior.
%
In our case, we can use integer programming to solve problems to optimality, so
it is an \emph{expert} in some sense.
%
The way integer programming solvers operate does not immediately fit our
constructive scheduling scheme, but we can simply \emph{backtrack} the states
and actions that are necessary to construct the optimal schedule.
%
By sampling some instance from the distribution of interest and replaying an
optimal route sequence on the MDP, while keeping track of the visited states.
This results in a set of state-action pairs. We are now in the usual supervised
learning setting, where we need to choose the parameters $\theta$ such that our
policy best fits this set of training data.

Let $\mathcal{I}_\mathrm{train}$ be some sample of training instances. For each
$\omega \in \mathcal{I}_\mathrm{train}$, let $\eta^\omega$ denote some optimal route sequence,
which can be found by solving~\eqref{eq:C'} using integer programming, and let $N(\omega)$
denote the total number of vehicles in $\omega$.
%
...
The imitation learning method relies on our ability to compute optimal
schedules. This assumption fails to hold relatively quickly, once we start
considering reasonably sized instances, which means that we do not have an
expert to rely on.
%
A central aim in the field of reinforcement learning is to automatically
discover good behavior from direct interaction with the system. In other words,
the goal is to \emph{learn from experience} rather than from demonstration.

Concretely, this means that we pick some initial $\theta$, execute $\pi_\theta$
on $\mathcal{M}_\mathrm{constr}$ a few times, evaluate how good the resulting
schedules turned out to be, and update $\theta$ such that it hopefully goes
better next time.
%
The magic is really in the last step: how can we change $\theta$ a little bit
such that $\pi_\theta$ improves?

A large part of the answer to this central question is given by the
fundamental Policy Gradient Theorem, see for
example~\cite{suttonReinforcementLearningIntroduction2018}.
%
Recall our learning objective~\eqref{eq:MDP}, and consider the expected reward
\begin{align}
  J(\theta) := \mathbb{E}_{\omega \sim P}[R(\eta(\omega, \pi_\theta))] ,
\end{align}
as a function of the parameter $\theta$. Note that the expectation is taken
over random instance samples as well as the random behavior of the policy.
%
The Policy Gradient Theorem essentially tells us that we can use samples of an
episode $\tau = (s_0, \eta_1, r_1, s_1, \dots, s_N, r_N)$ to estimate the gradient of
the average reward with respect to the parameter as
%
...
\end{align}
%
We can use this gradient approximation $\widetilde{\nabla_\theta J}$ to take a
little step in the direction in parameter space to increase the expected
performance, i.e., we perform the \emph{gradient ascent} update
\begin{align*}
  \theta \leftarrow \theta + \alpha \widetilde{\nabla_\theta J} ,
\end{align*}
for some small learning rate $\alpha > 0$.
%
A well-known manifestations of this approach is the classical REINFORCE
algorithm.

In practice, the gradient estimate~\eqref{eq:grad} suffers from a lot of
variance, which may hamper convergence.
%
% A classical way to prevent this is to use a so-called baseline estimate.
%
Much later work can be seen as ways of tackling this issue.
%
Our current purpose is not to understand the fundamentals of policy-gradient
algorithms in general, but rather to investigate the feasibility of this
approach in general. We will use the modern and popular Proximal Policy
Optimization (PPO) algorithm~\cite{schulmanProximalPolicyOptimization2017}. Very roughly speaking, this method keeps
the updated policy close to the current policy, by limiting how far the gradient
ascent step updates the parameter.

\addtocontents{toc}{\protect\pagebreak}
% \addtocontents{toc}{\protect\vspace*{2em}}
\section{Results and discussion}\label{sec:results}

In this section, we evaluate the quality of the heuristics proposed above by
presenting some numerical results on randomly generated problem instances.

\subsection{Experimental setup}

% performance metrics: optimality, training time, eval (inference) time
Algorithm performance is evaluated based on two aspects. Of course, the
...
to be careful here, because some of our methods involve both training time as
well as inference time; the main benefit of learned heuristics lies in the fact
that trade of longer training time, which is spend only once, for shorter
inference time.
...
% of instance size and the axis of instance structure.
...
with some common mean $\mu_r$.
%
We call $\lambda_r := (\mu_r + \rho)^{-1}$ the \emph{arrival intensity} of route
$r$, for the following reason.
%
Assume $n_r = \infty$ and let $N_t$ denote the counting process of the renewal
process $A_r = (A_{rk})_{k=1}^\infty$ with interarrival times $X_{rk} + \rho$.
%
In other words, $N_{t}$ counts the cumulative number of arrivals up to time $t$.
By the \emph{renewal theorem}, we obtain the limiting density of
arrivals
%
\begin{align*}
...
We fix two route arrival distributions $F_\mathrm{low}$ and $F_\mathrm{high}$,
modeling arrivals with low and high chance of vehicles occuring in platoons; we
achieve this by letting the interarrival times be distributed according
to~\eqref{eq:mixture} with parameters:
% in~Table~\ref{tab:distributions}.

% \begin{table}[h]
  % \caption{Two fixed route arrival distributions with the same arrival
  %   intensity.}\label{tab:distributions}
  % \centering
 \begin{center}
...
$n$ and $F$.
%
Given some problem instance $\omega \sim P$, let $\eta^{*}$ denote some optimal
route order, which we compute by solving the MILP problem~\eqref{eq:C'}, where
we use the two cutting planes of~\Cref{sec:cuts}. We use a fixed time limit of
two minutes per instance for the integer programming, in order to keep the total
analysis time within reason. Therefore, we might not always be able to obtain
the actual $\eta^{*}$ some of the larger instances; we use the best solution
found by the solver as a proxy. Given some route order $\eta$, we define its
\textit{optimality gap} as
\begin{align*}
  \mathrm{gap}(\omega, \eta) = \frac{R(\omega, \eta) - R(\omega, \eta^*)}{R(\omega, \eta^{*})} ,
\end{align*}
and for each trained policy $\pi$, which may be stochastic, we report the
...
We compare the following four solution methods:
\begin{enumerate}
  \item (MILP) For each problem distribution, we solve 50 instance with
        Mixed-Integer Linear Programming using the
        Gurobi\footnote{\url{https://www.gurobi.com}} solver with both cutting
        planes of~\Cref{sec:cuts}. We limit the running time to one minute per
        instance. We report the average objective $\mathrm{obj}$ of these
        instances and the average running time $T$ that is actually spend per
        instance. For the remaining methods, we report the gap with respect to
        these optimal solutions, as defined above.

  \item (Threshold) We use a grid search to fit the threshold heuristic
        from~\Cref{sec:threshold} by searching over 20 evenly spaced parameter
        values between $0$ and $1$. Because inference is so cheap for this
        heuristic, we use 500 training instances to evaluate the performance of
        each parameter value.

  \item (Imitation) Expert demonstration is obtained by solving 20 training
        instances with integer programming, with a running time limit of 20
        seconds per instance.
        %
        We perform imitation learning with the recurrent architecture
        from~\Cref{sec:neural-param}. The recurrent neural net is shared for
        each route and has a single hidden state of size 32, with tanh
        activation. The fully connected network has two layers of size 32, with
        ReLU activation.
        %
        Supervised learning is done in batches of 20, for a total of 2000 steps,
        using the Adam optimizer and a learning rate of 1e-4.

  \item (PPO) We use the Stable Baselines PPO implementation%
        %
        \footnote{\url{https://stable-baselines3.readthedocs.io/en/master/modules/ppo.html}}
        with the default policy network with ReLU activation.
        %
        Training is done for half a million steps.
\end{enumerate}

Our implementation of $\mathcal{M}_\textrm{constr}$ satisfies the interface of
the Gymnasium\footnote{\url{https://gymnasium.farama.org}} framework, so it can
be used with any reinforcement learning implementation that is compatible with
this.
%
This class \emph{SingleScheduleEnv} also contains a method for rendering the state of
the partial schedule, which was also used to generate the figures in this
report.
%
The abstract state function $\phi$ is implemented as a Gymnasium wrapper,
\emph{HorizonObservationWrapper}.
%
To avoid invalid actions when some route does not longer have any unscheduled
vehicles, so when it is done, we also implemented a
\emph{HorizonRollingWrapper}, that permutes---or ``rolls''---the observation and
actions in such a way that any action is always valid.
%
Due to limited time, we cannot specify more detail here, but it would be
interesting to verify whether this transformation is also a proper state
abstraction function.

The code is organized in a couple of Jupyter notebooks, which can be found at
\begin{quote}
  \url{https://github.com/jeroenvanriel/traffic-scheduling/tree/master/single}
\end{quote}
%
Data classes for instances and schedules, instance generation, and mixed-integer
programming is implemented in the \emph{basics.ipynb} notebook, so this would be a
good starting point to explore the code.
%
The implementation of $\mathcal{M}_\textrm{constr}$ and $\phi$ is found in
\emph{mdp.py}.
%
Experiment orchestration is handled from a single notebook
\emph{experiments.ipynb}.
%
Each method is implemented in its own notebook and we used the
Jupytext\footnote{\url{https://jupytext.readthedocs.io}} package to expose this
implementation for reuse in other notebooks. This means that some
\emph{notebook.ipynb} notebooks are synced to \emph{notebook.py} files, in
which all cells are exported, except those marked with the \emph{active-ipynb}
cell tag.


\subsection{Interpretation of results}

Table~\ref{tab:heuristics-results} contains the numerical results for
intersections with two and three routes, for various numbers of arriving
vehicles per route and the two route arrival distributions defined above.
%
This table took a couple of hours to compute. As can be seen from the table,
this time is mainly dominated by the integer programming for the six largest
distributions of instances, for which the time limit is almost always reached.
%
We stress that each of the heuristics has an evaluation time of less than a
second. In other words, we pay the price of the training time once, after which
the learned fast heuristic can be applied nearly instantly to any unseen
instance.

\paragraph{Threshold heuristic.}
The threshold rule clearly provide a heuristic that is cheap to fit and that
works well for almost all problem distributions.
%
This somewhat confirms our initial believe that there is a lot of structure in
single intersection instances. It also provides some additional verification
that our definition and implementation of the state abstraction function is
correct.

\paragraph{Imitation learning and policy-gradient.}
As expected for small instances, the imitation learning heuristic performs
always better than policy-gradient optimization, because the former has access
to more high-quality data.
%
It becomes increasingly expensive to compute this expert demonstration for
larger instances, so we had expected that policy-gradient would eventually
outperform the imitation learning heuristic for larger instances.
%
Although PPO provides a ready-to-use method that can handle instances of any
size, its performance clearly degrades for larger instances.
%
It is not clear at this moment whether the problem lies in our implementation of
$\mathcal{M}_\textrm{constr}$ and $\phi$, or in the policy architecture or
training hyperparameters.

We hypothesize that the recurrent embedding of~\Cref{sec:neural-param} could
provide a better alternative to the default policy network of the Stable
Baselines implementation.
%
We did experiment with this by implementing the recurrent architecture as a
\emph{custom feature extractor} within the Stable Baselines policy definition,
see \emph{ppo-learning.ipynb}, but this caused significantly worse convergence
compared to the defaults.
%
Alternatively, constructing a tailored-made critic or value estimate could help
reduce variance of the gradient estimate, thereby improving convergence.
%
We also experimented with a value estimate based on a roll-out of the threshold
heuristic, which is somewhat similar to a greedy rollout baseline method, see
Section 4 in~\cite{koolAttentionLearnSolve2019}.
...
\begin{sidewaystable}[p]
  ...
  \caption{%
    \emph{Performance comparison of heuristics with respect to integer programming baseline.}
    Each row corresponds to a problem distribution, specified by the number of
    vehicles per route $n_r$ and the type of arrival distribution 
    $F_\mathrm{train}$ that is used on all routes.
    %
    To evaluate the integer programming baseline, we sample and solve 50 instances to optimality, with a fixed time limit of one minute per instance.
    For each distribution, we indicate the average objective opt and the average solver time $T$.
    %
    For each heuristic, we list the average objective obj for the 50 test instances, the average optimality gap with respect to the optimal MILP solutions and the total training time $T_\mathrm{train}$.
    %
    The lowest gap is highlighted in each row.
    %
    The evaluation time of heuristics is not listed, because it is less than one 
    second in all cases.}\label{tab:heuristics-results}
% \begin{adjustbox}{center}
% \scalebox{1.0}{
% \input{experiments.tex}
% }
...
% \pagebreak
\subsection{Interpretation of using stochastic policies}

% stochastic transitions
It might seem strange to introduce stochasticity
in a completely deterministic problem environment.
%
Although the final policy should be deterministic, during the optimization
of~\eqref{eq:max-mdp-performance}, we consider stochastic policies.
%
The reasons for this are mainly of a technical nature: promoting exploration and
to make sure the policy gradient actually exists.
%
In some sense, this is similar to the practice of considering \emph{relaxations}
when solving problems with integer constraints. In the relaxed version of an
integer program, the integer constraints are removed, which makes it generally
much easier to solve. Of course, the final solution must satisfy the integer
constraints, so we need some sort of \emph{rounding procedure} to return to
integer values.
%
...
More generally, non-deterministic transitions of the MDP have been be used to
model certain parts of the computation that are too complex to model explicitly.
%
This has for example been done in existing joint methods of combinatorial
learning, in which some part of an existing combinatorial optimization solver is
augmented by replacing a specific component with a machine learning model.
%
...
%
In that case, the actions correspond to selecting some valid
...
%   rules} in the context of job shop scheduling.
...
Under some mild assumptions, some parts of the framework presented in the
previous chapters can be extended rather easily to model traffic networks with
multiple intersection.
%
Specifically, we introduce some simple graph notation to model grid-like
networks of intersections. To keep it simple, we assume that routes are fixed
and independent, in the sense that vehicles of different routes only meet at
intersection; there is no merging and splitting of routes.
%
We expect that this assumption keeps the combinatorial complexity of the problem
somewhat under control.

The trajectory optimization problem~\eqref{eq:T} has a very simple extension in to the
network setting.
%
Again, direct transcription is a relatively straightforward way to solve this
problem. However, this method does already pose scaling issues in the single
intersection, so without further modifications, we do not consider this method a
feasible approach for network-wide coordination.

\paragraph{Decomposition.}

We show how the bilevel decomposition of~\Cref{sec:decomposition} can
be extended to the network setting.
%
Similar to what was done in~\Cref{sec:delay-minimization}, we will assume that
the objective only involves delay and that vehicles are required to cross
intersections at full speed.
%
In networks, we can measure delay in more than one way. For example, we can
consider delay at every intersection, or at the last intesection of the
vehicle's route.
%
Like for the single intersection, these assumptions reduce the problem to a
crossing time scheduling problem and we can again employ integer programming as
a baseline solution method.

Compared to the single intersection setting, there is one additional
complicating factor. Until now, we did not take into account the maximum number
of vehicles that an approaching lane can handle. In the current setting, we can
no longer ignore the finite space of the lanes between intersection; we need to
explicitly model their \emph{finite capacity}.

It turns out to be nontrivial to describe how the capacity is affected by the
lane length and the vehicle dynamics.
%
...
see the two inequalities in~\eqref{eq:feasibility-inequalities}.
%
Our preliminary work towards a similar characterization is outlined
in~\Cref{chap:capacitated-lanes}.
%
For now, we present some conditions that are a little bit too conservative, but
guarantee that vehicle trajectories are safe.

\paragraph{Constructive scheduling.}

We briefly discuss the constructive approach for crossing time scheduling in
networks.
%
There was not enough time in this project to fully explore this direction, but
we did identify some fundamental issue that we think is worth mentioning.
%
With multiple intersections, each scheduling decisions involves picking both an
intersection and a route.
%
In other words, the scheduler has to pick an intersection to \emph{attend} and
then choose the route from which the next pending vehicle is scheduled.

Observe that it does not matter for the final schedule in which order intersections are
attended by the scheduler.
%
It is natural to expect that this redundancy slows the convergence of procedures
like imitation learning or policy-gradient and some quick numerical experiments
verify this.
%
Removing this redundancy is an interesting direction for further research.

\section{Network model and trajectory optimization}

\begin{figure}
  \centering
  \vspace{1.0em}
  \includegraphics[width=0.75\textwidth]{../network/figures/network-1}
  \caption{Stylistic illustration of our network model, showing some
    Manhattan-like traffic network with vehicle drawn as little red rectangle.
    There are $n=3$ rows and $m=4$ columns of intersections, and we assume there
    are $n+m$ routes: two from east to west and three from south to north. All
    roads are single-lane and one-directional without turning at intersections.}\label{fig:network_illustration}
\end{figure}

Consider a traffic network with straight lanes and intersections where turning
is not allowed, as illustrated in Figure~\ref{fig:network_illustration}.
% 
We define a directed graph $(\bar{V},A)$ with nodes $\bar{V}$ and arcs $A$, to
which we will refer as \emph{lanes}. Let $d(v, w)$ denote the length of lane
$(v,w)$.
%
Nodes with only outgoing arcs are \textit{entrypoints} and nodes with only incoming arcs
are \textit{exitpoints}. Let $V$ be the set of \textit{intersections}, which are all the nodes
with in-degree at least two.

This weighted graph encodes the possible routes $\mathcal{R}$ that vehicles can
follow.
%
Each route $r$ is defined as a path
\begin{align*}
  \bar{V}_{r} = (v_{r}(0), v_{r}(1), \dots, v_{r}(m_{r}), v_{r}(m_{r}+1)) .
\end{align*}
We require that the first node $v_{r}(0)$ is an entrypoint and that the last
node $v_{r}(m_{r}+1)$ is an exitpoint and we write
$V_{r} = \bar{V}_{r} \setminus \{ v_{r}(0), \, v_{r}(m_{r}+1) \}$ to denote the part of
the path that is restricted to intersections.
%
We say that some $(v, w) \in A$ is on path $V_{r}$ whenever $v$ and $w$ are two
consecutive nodes on the path and we write $A_{r}$ to denote the set of all
these arcs.

Let $\mathcal{N}$ to denote all the vehicles in the system and let
$\mathcal{N}_r$ to denote vehicles on route $r \in \mathcal{R}$.
%
To keep the model simple, we make the following assumption on routes:

\begin{assump}\label{assump:lane-disjoint}
For every pair of routes $p \neq q$, we have $A_{p} \neq A_{q}$.
\end{assump}
%
This assumption essentially says that routes can only overlap at intersections
and may not share lanes.
%
This \emph{lane-disjoint route} assumption has a huge impact on the
combinatorial complexity of the crossing order decisions, because it keeps the
problem \emph{local} in some sense.
%
The grid network of Figure~\ref{fig:network_illustration} satisfies this assumption.

This lane-disjoint route assumption ensures that the relative ordering of
...
$\mathcal{C}_r$ for those pertaining only to route $r$.
%
...
Appendix~\ref{app:A} could also be applied here, which would allow lanes that intersect at arbitrary angles.
%
However, for the sake of the current presentation, we consider networks in which
all roads are axis-aligned, such that intersections always involve perpendicular
lanes, like in Figure~\ref{fig:network_illustration}.
%
This means that conflicting vehicle positions can be modeled similar as
in~\Cref{chap:single}.
%
For each $k\in \{1, \dots, m_r\}$, we define the conflict zone
...
%
We will also use the shorthand notation $\mathcal{E}_r(v) := \mathcal{E}_{rk}$
with $k$ such that $v_r(k) = v$ to denote the positions of vehicles from route
$r$ that occupy intersection $v$.

\begin{figure}
  \centering
  \includegraphics[scale=1]{figures/motion/intersection}
  \caption{Part of a horizontal route with two intersections of length $W$. Each
    intersection area is marked with a little cross. The conflict areas
    $\mathcal{E}_{rk} = (B_{rk}, E_{rk} + L)$ are the positions of the front of
    the vehicle that correspond to occupying some intersection. Hence, the four
    shaded rectangles illustrate the positions where the vehicle just enters of
...
Under the assumptions above, the single intersection trajectory
optimization problem~\eqref{eq:T} almost immediately extends to the network case.
%
We assume that vehicles satisfy the motion dynamics of~\Cref{chap:single}.
%
Like before, we write $x_i \in D(s_i)$ to denote that $x_i$ is a valid
trajectory for vehicle $i$ with initial conditions $s_i$.
%
Let the joint initial state of all vehicles in the system be denoted by
$s = \{ s_i = (x_i^0, v_i^0) : i \in \mathcal{N} \}$.
%
Let $J(x_i)$ be some trajectory performance criterion, for example
$J_{\alpha,\beta}$ or $J_{v_d}$ defined in~\Cref{sec:criteria}.
%
Assume the network $(V,A)$ and system parameters
$(L, \mathcal{E}_r, \bar{v}, \omega, \bar{\omega}, t_f)$ are fixed, with $t_f$ some final
time and let $x := \{ x_i : i \in \mathcal{N} \}$ denote the joint position of all
vehicles. Consider the following optimization problem over $x$.
%
...
Like before, the trajectory optimization problem~\eqref{eq:N} can be immediately
solved by direct transcription to an integer (potentially nonlinear) program.
%
However, without further tricks, this approach does not scale to networks of
practically relevant sizes, because the large number of variables involved.
%
Instead, we try to obtain a proper decomposition like we did
in~\Cref{sec:decomposition}.
%
The general bilevel reformulation poses no problems. We again assume that
vehicles cross intersections at full speed and that performance is measured only
in terms of delay, which should enable a proper decomposition.
%
However, it turns out to be tricky to precisely define the resulting crossing
time scheduling problem.
%
We provide a formulation that is very close; we think that it is a little bit
too conservative in some sense.
...
\end{align*}
be the crossing time and exit time, respectively. We also define
$\sigma(i, v) = z(i, v) - y(i, v)$, which is the amount of time vehicle $i$ occupies
intersection $v$.
%
...
set of feasible parameters, respectively, of the parametric lower-level problems
%
\begin{alignat}{3}
  ...
  & x_{i}(t) - x_{j}(t) \geq L , && \quad \text{ for } (i, j) \in \mathcal{C}_{r} , \tag{NL.2} \\
...
\end{alignat}
where we again use subscript $r$ to group variables according to their
associated route.
%
A rigorous proof of the correctness of the above reformulation would be a
trivial extension of the proof presented for Theorem~\ref{thm:decomposition} and does not provide
much further insight.

\subsection{Proper decomposition}
In order to fully separate the upper level problem, we make the following
assumptions and definitions, which parallel those in~\Cref{sec:delay-minimization}.
...
\begin{assump}[Uniform intersection]
  ...
  $E_{rk} - B_{rk} = E_{r'k'} - B_{r'k'}$, for all $r, r'$ and $k,k'$. When
  vehicles drives at full speed on the full intersection area, it occupies the
  intersection for a duration of precisely
  \begin{align}
    \sigma := (E_{rk} + L - B_{rk}) / \bar{v} ,
  \end{align}
  Again, define the \emph{processing time} $\rho := L / \bar{v}$ and let $\delta := \sigma - \rho$
  denote the \emph{switch-over time}.
\end{assump}

\begin{define}[Earliest crossing time]
  Let $i$ be some vehicle on route $r$ with trajectory $x_i$ and initial state
  $s_i = (x_i^0, \bar{v})$. Define the \emph{earliest crossing time} at intersection
  $v \in V_{r}$ to be
  \begin{align}
...
\begin{define}[Delay criterion]
  ...
  \begin{align}
    J_d(x_i) = y(i, v_r(m_r)) - a(i, v_r(m_r)) .
  \end{align}
  Instead, we could also take the sum of delay experienced at each intersection:
  \begin{align}
    \bar{J}_{d}(x_i) = \sum_{v \in V_{r(i)}} y(i, v) - a(i, v) .
  \end{align}
\end{define}

\paragraph{Isolating the timing problem.}
Using a similar argument as in~\Cref{sec:delay-minimization}, it can be shown
that the full speed crossing assumption causes the feasibility of the
lower-level problems to depend only on the crossing time schedule $y$.
%
This means that we have $z(i, v) = y(i, v) + \sigma$.
%
Let $ y_r := \{ y(i, v) : i \in \mathcal{N}, v \in V_{r} \}$, then we can focus on the
set of feasible crossing times
\begin{align}
  \mathcal{Y}(s_r) := \{ y_r : (y_r, y_r + \sigma) \in \mathcal{S}(s_r)  \} .
\end{align}

It is not difficult to see that each lower-level trajectory optimization problem
decomposes even further into a sequence of smaller problems.
%
For each two consecutive intersection along $V_{r}$, we can consider the lower-level trajectory optimization problem separately.
%
This means that $y_{r} \in \mathcal{Y}(s_{r})$ is equivalent to
$y_{e} \in \mathcal{Y}_{e}(s_r)$ for each lane $e = (v,w) \in A_{r}$, where $y_{e}$
denotes the vector of all variables $y(i, v)$ and $y(i, w)$ for all
$i \in \mathcal{N}_{r}$, and where $\mathcal{Y}_{e}(s_r)$ denotes the set of
values of $y_{e}$ for which a feasible trajectory part can be found on this part
of the path.

Unfortunately it is non-trivial to formulate a polyhedral characterization of
$\mathcal{Y}_e(s_r)$, like in~\Cref{sec:delay-minimization}.
%
Without rigorous proof, we now provide the following sufficient conditions:
%
\begin{subnumcases}{\hspace{-3em} y_e \in \mathcal{Y}_e(s_r) \iff }
 \;   a(i, v) \leq y(i, v) & for $i \in \mathcal{N}_r$, \tag{arrival} \\[4pt]
 \;   y(i, v) + \rho \leq y(j, v) & for $(i, j) \in \mathcal{C}_r $, \tag{follow} \\[5pt]
 \;   y(i, v) + d(e) / \bar{v} \leq y(i, w) & for $i \in \mathcal{N}_r$, \label{travel} \tag{travel}  \\[5pt]
 \;   \check{y}(i, v) \leq y(i, v)  & for $i \in \mathcal{N}_r$, \label{buffer} \tag{entry}
\end{subnumcases}
%
where $d(e)$ denotes the length of lane $e = (v,w)$.
%
The first two constraints are not very surprising; they merely extend their
single-intersection counterparts in terms of notation.
%
We refer to the third constraint as the \emph{travel constraint}, since it encodes the
minimum time necessary for some vehicle $i$ to travel accross $e$.

\paragraph{Entry space constraint.}

We use the constraint (entry) to model the fact that there must be enough space
in the lane when a new vehicle enters, so we refer to it as the \emph{entry space
  constraint}.
%
At this moment, we do not know how to formulate $\check{y}(i, v)$ such that the
four constraints together are necessary and sufficient for
$y_e \in \mathcal{Y}_e(s_r)$.

Nevertheless, we are pretty confident that by setting
\begin{align}\label{eq:check-y}
  \check{y}(i, v) := y(i - c(e), w) + (c(e) L - d(e)) / \bar{v} ,
\end{align}
%
we obtain an inner approximation of $\mathcal{Y}_e(s_r)$, meaning that the
constraints on the right are sufficient.
%
In this expression, $c(e)$ denotes the capacity of the lane, which we define to
be the maximum number of vehicles that can enter the lane, queue up at the end
of the lane, with just enough room to accelerate to a full speed crossing. It is
thus computed as
\begin{align}
  c(e) = \left\lfloor \frac{d(e) - 1/(2\bar{\omega}) - 1/(2\omega)}{L} \right\rfloor + 1 ,
\end{align}
where we recall that $L$ is the minimum distance required between two
consecutive vehicles, and where $1/(2\bar{\omega})$ is the minimum time to
accelerate from a standstill to maximum speed and, similarly, $1/(2\omega)$ is the minimum
time to fully decelerate from full speed to standstill.
%
In its current form,~\eqref{eq:check-y} is a little bit too conservative; some
quick numerical simulations show that vehicles are able to enter the lane
slightly earlier.
%
Our preliminary work on making the above discussion more rigorous is presented
in~\Cref{chap:capacitated-lanes}.

\subsection{Integer programming}

Using the above four constraints with~\eqref{eq:check-y} means we lose a little
bit of optimality, but we suspect that the impact is very limited in practice.
%
Therefore, we will use these sufficient conditions to formulate the following
crossing time scheduling problem in networks, which is the direct extension
of~\eqref{eq:C}.
%
By encoding the crossing order constraints with binary variables with the
big-$M$ trick this problem turns into a mixed-integer linear program.

\begin{alignat}{3}
  \min_{y} & \sum_{i \in \mathcal{N}} \sum_{v \in V_{r(i)}} y(i, v) \tag{NC} \label{NC} \\
  \text{ s.t. } & a(i, v) \leq y(i, v) && \text{ for } i \in \mathcal{N}, v \in V_{r(i)} , \tag{NC.1}\label{eq:travel} \\
                & y(i, v) + \rho \leq y(j, v) && \text{ for } (i, j) \in \mathcal{C}, v \in V_{r(i)} , \tag{NC.2} \\
                & y(i,v) + \sigma(i,v) \leq y(j,v) \text{ or } \quad \notag \\
                & y(j,v) + \sigma(j,v) \leq y(i,v) , && \text{ for } \{i,j\} \in \mathcal{D}^{v}, v \in V, \tag{NC.3} \label{eq:network-disj} \\
                & y(i, v) + d(v,w) / \bar{v} \leq y(i, w) && \text{ for } i \in \mathcal{N} , (v,w) \in A_{r(i)}, \tag{NC.4} \label{eq:travel} \\
                & \check{y}(i, v) \leq y(i, v) && \text{ for  } i \in \mathcal{N}, (v,w ) \in A_{r(i)} . \tag{NC.5} \label{eq:buffer}
\end{alignat}

\paragraph{Runtime benchmark.}

We evaluate how well general-purpose solvers scales to larger network sizes by
performing the following little benchmark.
%
For some given grid network with $m$ rows and $n$ columns of intersections, with
a fixed number of arrivals $n_r$ for each of the $m+n$ routes, we let the solver
run for some fixed time per instance.
%
We record the final MIP gap reported by the solver after this fixed time.
For minimization problems, the gap is roughly defined as
\begin{align*}
  gap := \frac{\textit{obj} - \textit{bnd}}{\textit{obj}} .
\end{align*}
where \emph{obj} is the objective of the best solution found so far and
\emph{bnd} the best lower bound on the objective so far.

The results are presented in Table~\ref{tab:milp-bench-results}. Observe that
the results are more dependent on $N$ than on the network size, which is not
really surprising, because the number of conflicts does not really depend on the
network, due to Assumption~\ref{assump:lane-disjoint}.
%
It would be interesting to see how removing this assumption affects the quality
of solutions found by integer programming.

\begin{table}[!hp]
  \caption{Benchmarking the final MIP gap of integer programming for crossing
    time scheduling in networks of varying sizes. We record the average MIP gap
    with standard deviation over 5 instances, after running the solver for at
    most a minute on each instance. The network size is written as rows $\times$
    columns. $N = n_r (n+m)$ denotes the total number of vehicles in the network.
    For this benchmark, we used the Gurobi solver version 11.0.2 on a system
    with a 13th Gen Intel i5-13600K CPU and 32GiB of RAM.}\label{tab:milp-bench-results}
  \centering
  \hspace{-0.8em}
  \input{milp-bench.tex}
\end{table}

\section{Constructive scheduling}

We now briefly discuss some of our preliminary findings on extending the
constructive scheduling approach of~\Cref{chap:single-learning} to networks of
intersections.
%
...
\paragraph{Neural constructive heuristics.}

The recurrent neural network parameterization of~\Cref{sec:neural-param} can be extended to
networks.
%
...
Alternatively, we can consider one of the fixed intersection orders described
above (random, boundary, alternating).

Table~\ref{tab:results} contains some of our preliminary numerical results for
this imitation learning setting. It seems to verify our hypothesis that the
order in which intersections are attended matters, with the alternating order
outperforming the other two.

\input{data-network/comparison-woven.tex}

\subsection{Similarity to job shop scheduling}

Observe that the crossing time scheduling problem~\eqref{NC} is quite similar to
Job Shop Scheduling Problems (JSSPs), see Appendix~\ref{app:job-shop} for some
background on this classical class of problems.
%
The intersections can be interpreted as machines and the vehicles as jobs that
need to visit the machines in a certain order. Some part of the this visiting
order is already fixed, for vehicles from on the same route.
%
The reason why this parallel is important is because job shop has received a lot
of attention\footnote{Pun not intended: a lot of recent proposals involve 
  neural attention architectures.} in the literature and various ways of
combinatorial learning have been proposed.
%
Hence, it is interesting direction of further research to leverage the recent
advances in this field by trying to apply similar techniques to~\eqref{NC}.
%
We will now highlight three publications that caught our attention.
%
A lot of works in this direction rely on some kind of graph neural network
architecture. The survey~\cite{smitGraphNeuralNetworks2024} provides an overview
based on this distinguishing feature, and it serves as a good general starting
point to explore this research area.
...
designed features. They consider the makespan objective and propose a dense
...
The Learning to Dispatch (L2D) paper~\cite{zhangLearningDispatchJob2020}
proposes a constructive scheduling heuristic for JSSP with makespan objective.
Their method is based on a dispatching policy that is parameterized in terms of
a graph neural network encoding of the disjunctive graph of the partial
schedule.
%
Each action corresponds to choosing for which job the next operation is
dispatched. Rewards are based on how much the lower bound on the makespan
changes between successive states. They use a Graph Isomorphism Network (GIN)
architecture to parameterize both an actor and critic, which are trained using
...
More recently, the authors of L2D investigated an improvement heuristic for
JSSP~\cite{zhangDeepReinforcementLearning2024} with makespan objective.
%
This method is based on selecting a solution within some neighborhood of the
...
this form.
%
We use $A$ and $B$ to denote the start and end position of the lane.
%
...
\begin{figure}
  ...
  \caption{Example trajectories $x_{1} \in \mathcal{D}[a_{1},b_{1}]$,
    ...
    of the three classes of trajectories that are used throughout this chapter
...
The analysis of feasibility conditions in this chapter is somewhat related to
...
%
They study an online setting, in which new vehicles arrive to the system at
...
Coordination of autonomous vehicles at intersections poses challenging
computational problems that have been approached from many perspectives across
different research communities---each bringing their own modeling paradigms and
solution methodologies.
%
We have taken a deliberately simplified yet rigorous mathematical optimization
perspective, focusing on understanding the fundamental computational challenges
that arise even in idealized settings.
%
By abstracting away from practical complexities---unobservability, decentralized
control, and mixed traffic---we could isolate and analyze the core computational
difficulty: \emph{the joint optimization of discrete crossing order decisions and continuous-time vehicle
  trajectories}.

One of the central themes in our discussion has been the decomposition of this
joint optimization problem into an upper-level scheduling problem that
determines when vehicles cross intersections, and lower-level trajectory
generation problems that compute the actual vehicle paths.
%
Under specific assumptions---particularly that vehicles cross intersections at
full speed and that delay minimization is the primary objective---we found that
...
This reduction opened the door to applying both established optimization
...
scheduling---provides a foundation for further investigation of methods that
scale to networks of intersections, though there are some remaining challenges
that merit further investigation.

\section{Contributions}

We summarize our key contributions and reflect on the research questions
of~\Cref{chap:introduction}.

\paragraph{Motion planning model with direct transcription baseline.}
We proposed a simple model of a single intersection with a simple extension to
...
Within this framework, we showed how the collision-avoidance constraints can be
directly encoded by direct transcription to an integer program, yielding a
straightforward solution method for the single intersection as well as for
networks of intersections.
%
Although this method guarantees to find optimal solutions that are provably
safe, it scales very poorly to larger problem.
%
Hence, without further changes, it is not a good candidate for large-scale
coordination in networks.
%
This issue motivated the remaining contributions.

\paragraph{Decomposition and improved integer programming.}

In the search for alternatives to direct transcription, we discussed the common
problem reformulation that separates the decisions regarding occupancy time slot
reservations.
%
...
demonstrated by data-driven approximation methods~\cite{hultApproximateSolutionOptimal2015,hultTechnicalReportApproximate}.
...
safe coordination scheme that has better potential for scaling up.
%
This part of the thesis answers RQ1.

% exploiting optimal structure
Next, we focused on solving the crossing time scheduling problem. Integer
programming can handle single intersection problems fairly well, but optimal
solutions become expensive for larger instances, without additional
modifications.
%
% Nevertheless, modern solvers provide a pretty good heuristic when computation
% time is limited.
% cutting planes
To further improve the applicability of integer programming, we showed how a
natural structure of optimal schedules leads to the formulation of two types of
cutting planes.
%
We find that adding both types of cuts significantly improves the speed of the
procedure.
%
This part of the thesis answers RQ2.

\paragraph{Hand-crafted and machine-learned constructive heuristics.}

As an alternative to integer programming, we explored a constructive scheduling
approach by defining a Markov decision process that simulates a step-by-step
construction of schedules for the single intersection.
%
We found some initial redundancy in this model, which motivated the definition
of a state abstraction function, resulting in a significant reduction of the
state space.

Using an elementary threshold rule, we were able to obtain surprisingly good
heuristics, demonstrating the effectiveness of the state reduction.
%
We investigated the potential for machine-learned heuristics by using neural
policy architectures, where the observation consists of the sequences of
upcoming arrivals for each route. We evaluated two methods of parameter
optimization:
%
when instances are relatively small, imitation learning is a natural choice; for
tackling larger instance sizes, reinforcement learning seems a good idea.
%
However, our results indicate that both methods with generic parameterizations
are outperformed by the simple threshold heuristic.
%
This part of the thesis answers RQ3.

\paragraph{Scaling up to networks of intersections.}

We partially answer RQ4. The main issue that we identified is the relative
difficulty of formulating the feasibility of the lower-level trajectory problems
in terms the upper-level crossing time schedule.
%
\Cref{chap:capacitated-lanes} provides a first attempt at rigorously analyzing
this problem and provides a conjecture about a characterization of the
feasibility as a system of linear inequalities in terms of the crossing times.

Using a crude approximation of feasibility, we took some first steps towards
combinatorial learning heuristics in the network setting
in~\Cref{chap:network-learning}.
%
...
next scheduling step, which introduces additional redundancy.
%
With imitation learning, we observed that the order of visiting intersections
matters and confirm our hypothesis that keeping the amount of progress---in
terms of scheduling steps taken---balanced across intersection, leads to better
performance.


\section{Limitations and further work}

We discuss some limitations, and outline directions for future research that
could help bridge the gap between our idealized models and real-world autonomous
traffic management systems.

% \paragraph{Heterogeneous traffic.}

% Our assumption of homogeneity of vehicle geometry and dynamics has been
% convenient in terms of presentation.
% %
% Large parts of our methodology are readily applicable when
% heterogeneity is considered.
% %
% However, the platoon preservation theorem fails to hold.
...
%
However, from a methodological standpoint, this setting is not satisfying.
%
In general, the trajectory optimization problem does not decompose: the upper-
...
\begin{enumerate}
  \item Feasibility of the lower-level problem is a complicating factor, because
        ...
        %
        We wonder how sampling-based methods can be leveraged to approximate
        these constraints.
        %
  ...
  \item In general, the optimization objective is not just a linear function of
        the upper-level decision variables.
        %
        We would like to see how a data-driven approximation of the objective,
...
to the platoon preservation theorem in small networks with only a couple of
intersections.
...
% \titleformat{\chapter}[block]   % default display style
%   {\normalfont\large\bfseries}   % font size & bold
%   {\thechapter.}   % “Chapter <number>”
%   {20pt}                        % spacing between number and title
%   {}
% \titlespacing*{\chapter}{0pt}{0.5em}{1em} % spacing: left, before, after

% \titleformat{\chapter}
%   {\normalfont\Large\bfseries} % font: Large and bold
%   {Appendix \thechapter~--}{0.4em}{} % label format
% \titlespacing*{\chapter}{0pt}{0em}{1em} % spacing: left, before, after

% \let\oldcleardoublepage\cleardoublepage
% \renewcommand{\cleardoublepage}{}  % temporarily disable new pages
...
% Counter for appendix sections
\newcounter{appsec}
\renewcommand{\theappsec}{\Alph{appsec}}

% Command for appendix sections
\newcommand{\appsection}[1]{%
  \refstepcounter{appsec}%
  \section*{\theappsec. #1}% unnumbered in the document
  \addcontentsline{toc}{section}{\protect\numberline{\theappsec.} #1}% appears in TOC
  \label{app:\theappsec}% reference label
}

% \addtocontents{toc}{\protect\addvspace{2em}}
\chapter*{Appendix}
\addcontentsline{toc}{chapter}{Appendix}
\addtocontents{toc}{\protect\addvspace{0.3em}}

\appsection{Feasible configurations in single intersection model}\label{app:configuration-space}

We derive the feasible configurations of two vehicles in a single intersection,
consisting of two routes intersecting at some arbitrary angle, as shown in
Figure~\ref{fig:intersection-non-axis-aligned}.
% general case
...
\appsection{Job shop scheduling and disjunctive graph}\label{app:job-shop}
...
%
Each job $j$ visits all machines\footnote{When some job $j$ requires only processing on a proper subset of the machines, observe that we can simply
...
\begin{figure}
  \centering
  \includegraphics[scale=1.0]{figures/job-shop-delay.pdf}
...
Job shop problems are notoriously hard to solve in practice. The class of
...
\end{align*}
which is generally referred to as the \emph{big-M method}.


\appsection{Local search for crossing time scheduling}\label{sec:local_search}
...
Consider for example the route order
...
% \section{Neural combinatorial optimization}\label{app:nco}
...
% This section reviews the literature on combinatorial learning, which has gained
% a lot of attention\footnote{Pun not intended: a lot of recent works rely on neural attention architectures.} recently. One of the key ideas in this line of research is
% to treat problem instances as data points and to use machine learning methods to
% approximately map them to corresponding optimal solutions~\cite{bengioMachineLearningCombinatorial2020}.
...
\appsection{Miscellaneous}\label{app:misc}
...
% \titleformat{\chapter}[display] % default display style
%   {\normalfont\Huge\bfseries}   % font size & bold
