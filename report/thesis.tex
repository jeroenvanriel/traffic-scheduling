...
\author{\Large Jeroen van Riel \\[6em] {\it Combined Master's Thesis} \\[1em] Industrial and Applied Mathematics \\ Computer Science and Engineering  \\[3em] {\it Supervisors} \\[1em] Marko Boon \\ Stella Kapodistria \\ Mykola Pechenizki \\ Danil Provodin  \\[8em] October 2025 \\[0.8em] Eindhoven University of Technology}
\date{}
...
%
Once we start considering multiple intersections, this scaling issue becomes
even more prominent, so we first try to obtain fast algorithms for the single
intersection crossing time scheduling problem with large numbers of vehicles.
%
Specifically, in this chapter, we are interested in developing heuristics to
obtain good schedules, with close to optimal delay, in limited time.
...
%
This structural insight can be precise, like the result of
Theorem~\ref{thm:exhaustive}, but more often than not it is based on some
hard-to-define intuition about what optimal schedules should look like, which
guides the design of the heuristic.
%
...
We start by showing that solutions to~\eqref{eq:crossing_time_scheduling} can be viewed as a sequence of route
indices. The idea is that we only have to decide in which order vehicles are
able to cross the intersection, and since the relative route of vehicles on the
same route is already fixed, this leaves us with ordering the routes.
%
...
%
First, we use this sequence representation to illustrate a simple local search
heuristic.
%
Next, we show how to construct a probabilistic model over schedules, by
assigning a probability to every solution sequence, conditioned on the problem
instance specification.
%
Our approach belongs to the class of principal methods mentioned above.
%
After precisely stating two different model parameterizations, we will show how
to use randomly generated problem instances for model fitting.
%
There are two general approaches for this: the most natural way is to use an
existing solver to compute optimal solutions, then use these as training data
points in a supervised learning setting.
%
However, this assumes that we are able to solve the combinatorial problem in the
first place.
%
An alternative approach is to ``learn'' how to construct such optimal sequences
by trial-and-error, by evaluating the current model to determine the next route
in the sequence, and then updating the model parameters based on an intermediate
score, which is best understood in the framework of reinforcement learning.

% choose disjunctive constraints -> linear program
\section{Sequence representation of schedules}

The crossing time scheduling problem is the starting point of our discussion, so
for ease of reference, let us first restate its MILP formulation
% \begin{subequations}
% \begin{alignat}{2}
%   \min_{y} \quad & \sum_{i \in \mathcal{N}} y_i \tag{C} \\
%   \text{ s.t. } \quad & a_{i} \leq y_i  && \quad \text{ for all } i \in \mathcal{N} , \tag{C.1} \\
%                     & y_i + \rho \leq y_{j}  && \quad \text{ for all } (i,j) \in \mathcal{C},\tag{C.2} \\
%                     & (y_{i}, y_i + \sigma) \cap (y_{j}, y_j + \sigma) = \varnothing && \quad \text{ for all } \{i,j\} \in \mathcal{D} \tag{C.3}.
% \end{alignat}
% \end{subequations}
% and its MILP encoding
\begin{equation}\tag{C'}
\renewcommand{\arraystretch}{1.2}
\begin{NiceArray}{ r l @{} >{{}}c<{{}} @{} l @{} }
  \displaystyle \min_{y,\gamma} & \displaystyle \sum_{i \in \mathcal{N}} y_{i} \\
...
decision variables $y_i$ are real-valued.
%
We will show that there is a more efficient representation of solutions.
%
Recall that the set of binary variables $\gamma_{ij}$ encode the crossing order
of all vehicles;
%
we will essentially show that these binary decisions contain all the necessary
information to encode the complete solutions $y$.
%
Specifically, after fixing the binary decision variables, we obtain a linear
program, which can be shown to have a unique solutions.

% introduce disjunctive graph
To make this more precise, it will be convenient to introduce the
\emph{disjunctive graph} representation of problem instances and solutions.
%
The basic idea is to encode each constraint as a weighted arc in a directed
graph.
%
...
$w(i,j) := \rho$.
%
The remaining constraints can be interpreted as choosing which of the
constraints $y_i + \sigma \leq y_j$ and $y_j + \sigma \leq y_i$ must hold for
each conflict $\{i,j\} \in \mathcal{D}$, which we encode as choosing whether we
include the arc $i \rightarrow j$ or the arc in the opposite direction $j \rightarrow i$; both of
these \emph{disjunctive} arcs have weight $w(i,j) := \sigma$.
%
For a little bit more background on the disjunctive graph formalism and seeing
it applied to a slightly more general scheduling problem, we refer the reader to
the discussion of the job shop scheduling problem in
Appendix~\ref{app:job-shop}.

% choosing disjunctive arcs
Instead of setting binary variables $\gamma_{ij}$ we can now equivalently think about
choosing at most one arc from every pair of disjunctive arcs to include in the
disjunctive graph. We use $\mathcal{O}$ to denote such a selection of
disjunctive arcs and let $G(\mathcal{O})$ denote the corresponding disjunctive
graph.
%
Whenever $\mathcal{O}$ contains precisely one of the disjunctive arcs for each
conflict, we say that $\mathcal{O}$ and $G(\mathcal{O})$ are \emph{complete}.
%
It will be convenient later to also allow partial selections $\mathcal{O}$, in
the sense that there may be conflicts for which no arc is chosen.
%
In the extreme case of $\mathcal{O} = \varnothing$, we obtain the so-called \emph{empty}
disjunctive graph $G_0 := G(\varnothing)$.

Given some choice of disjunctive arcs $\mathcal{O}$, we consider the so-called \textit{active}
schedule $y(\mathcal{O})$, which is defined as the solution to the linear
program
...
\end{subequations}
%
This linear program is not necessarily feasible for every choice of disjunctive
arcs $\mathcal{O}$, so the active schedule $y(\mathcal{O})$ might not always be
defined.
%
To give a concrete example where it fails to exist, suppose that the disjunctive
graph $G(\mathcal{O})$ contains some cycle
\begin{align*}
...
\end{align*}
such that we obtain an obvious contradiction when $\sigma, \rho > 0$.
%
This shows that the absence of cycles is necessary for active schedule
$y(\mathcal{O})$ to exist. Our next goal is to show that the absence of cycles
is also sufficient for existence and uniqueness of $y(\mathcal{O})$. Before
that, we first show the following node ordering property.

\begin{lemma}\label{lemma:vehicle-order}
  Assume that $G(\mathcal{O})$ is an acyclic complete disjunctive graph, then
  there exists a unique topological order $\pi$ of the (non-dummy) nodes
  $\mathcal{N}$.
\end{lemma}

A node in a directed acyclic graph that has no incoming arcs is called \emph{minimal}.
%
For the proof, we essentially pick and remove a minimal node from the
disjunctive graph to construct the sequence $\pi$. Therefore, the following
standard property of directed acyclic graphs will be convenient.

\begin{lemma}\label{lemma:minimal-node}
  Let $\mathcal{G}$ be some Directed Acyclic Graph (DAG) over nodes $V$, then
  there must be some node $v \in V$ that is minimal, so which has no incoming
  arcs.
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof of Lemma~\ref{lemma:vehicle-order}]
%
Let us first recall some of our notation for route and vehicle indexing. Given
some vehicle index $i = (r,k) \in \mathcal{N}$, we use $r(i) = r$ to denote its
route and $k(i) = k$ to denote its relative order on this route. Recall that we
use $\mathcal{N}_r := \{ i : r(i) = r\}$ to identify all vehicles on route $r$.

Starting from the complete disjunctive graph $\mathcal{G}_0 := G(\mathcal{O})$
we will argue that there must be a unique minimal node $\pi_1 \in \mathcal{N}$, so
without any incoming arcs.
%
...
Next, we show that $\pi_1$ must also the unique minimal node of $\mathcal{G}_0$.
%
...
are left with a sequence $\pi = (\pi_1, \dots, \pi_{|\mathcal{N}|})$. \note{[ define
  $N = |\mathcal{N}|$ earlier on ]}
\end{proof}

\begin{define}
  The sequence $\pi$ given by Lemma~\ref{lemma:vehicle-order} is called the
  \emph{vehicle order} for $G(\mathcal{O})$. Since the relative order of
  vehicles on the same route is already fixed, we can equivalently consider the
  \emph{route order} $\eta = (\eta_1, \dots, \eta_{|\mathcal{N}|})$, which is
  defined as $\eta_t(\pi_t) = i$ for $\pi_t = (i,k)$ for every
  $t \in \{ 1, \dots, |\mathcal{N}| \}$.
\end{define}

We are now ready to state the existence and uniqueness of $y(\mathcal{O})$,
which we establish through a unique sequential construction, essentially
following the path
$\pi = (\pi_1, \pi_2, \dots, \pi_t, \dots, \pi_{|\mathcal{N}|})$ through the
disjunctive graph $G(\mathcal{O})$ and computing $y_i$ at each step $i = \pi_t$
along the way.

\begin{lemma}\label{lb_lemma}
  Let $\mathcal{O}$ such that $G(\mathcal{O})$ a complete acyclic disjunctive
  graph, with unique vehicle order $\pi$. Assume that $\sigma > \rho$, then schedule
  $y = y(\mathcal{O})$ exists and is uniquely defined through
\begin{align}
  \label{eq:lb_lemma}
...
To conclude the above discussion, any valid selection $\mathcal{O}$ of
disjunctive arcs is equivalent to a sequence $\pi$ of vehicle indices. Observe
that it is also equivalent to a sequence $\eta$ of routes, because the ordering of
vehicles on the same route stays fixed. See Figure~\ref{fig:disjunctive_graphs} for an example of a
complete disjunctive graph with its corresponding vehicle order $\pi$ and route
order $\eta$. From now on, we will mainly be working with $\eta$, because it is in
some sense the simplest representation of a schedule and we write $y^{\eta}$ to
denote the corresponding induced crossing time schedule.
...
instance specification, which consists of the vehicle indices and the set of
earliest arrival times $a := \{ a_i : i \in \mathcal{N} \}$.
...
%
More concretely, we will consider so-called \emph{autoregressive} models of the
form
\begin{align}
...
Lemma~\ref{lb_lemma} is very much related to the notion of an \emph{active schedule}, see~\cite[Definition 2.3.3]{pinedoSchedulingTheoryAlgorithms2016}
...
\note{Also emphasize that we develop an explicit way to calculate optimal
  trajectories under the haste objective, for general arrival/crossing times
  (cf. Timmerman, Joshi).}
