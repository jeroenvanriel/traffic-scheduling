...
%% set pdf meta-data
\hypersetup{
    pdftitle = {Efficient and Provably Safe Autonomous Intersection Coordination: Combinatorial Learning in Network-Wide Motion Planning},
...
% \author{\Large Jeroen van Riel \\[4em] {\it Combined Master's Thesis} \\[0.6em] Industrial and Applied Mathematics \\ Computer Science and Engineering  \\[3em] {\it Supervisors} \\[0.6em] Marko Boon \\ Stella Kapodistria \\ Mykola Pechenizkiy \\ Danil Provodin  \\[4em] October 2025 \\[0.6em] Eindhoven University of Technology}
% \date{}
% \newcommand{\mytitle}{%
%   {\huge\bf Efficient and Provably Safe Autonomous Intersection Coordination}\\[1.2em]
%   {\Large\bf Opportunities of combinatorial learning\\[-0.3em] in network-wide motion planning}}
% \title{\mytitle}


\title{%
  {\huge\bfseries Efficient and Provably Safe Autonomous Intersection Coordination}\\[1em]
  {\Large\bfseries Combinatorial Learning in\\[-0.3em] 
  Network-Wide Motion Planning}
}
...
involving questions of how to adopt autonomous vehicle for safe~\cite{hilaryMakingAutomatedVehicles2022},
...
\begin{itemize}
  ...
  \item \emph{Decentralized control and communication.} A key design issue is
        how to organize communication and control within such systems.
        %
...
\begin{enumerate}[label=RQ\arabic*:, leftmargin=3em, rightmargin=4em]
  ...
  \item How well do established methods solve the crossing order problem?
        (complexity)
...
\begin{figure}
  ...
  \begin{subfigure}[b]{1.0\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../single/figures/milp-bench-small.pdf}
  ...
  \begin{subfigure}[b]{1.0\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../single/figures/milp-bench-large.pdf}
...
Recent research has increasingly explored the use of machine learning (ML)
techniques for tackling combinatorial optimization (CO) problems.
%
This chapter begins with an overview of the main ideas and methods in this area,
%
Based on this, the rest of this chapter details our own version of this
methodology for solving the crossing time problem introduced in~\Cref{chap:single}.
...
%
The presentation here draws inspiration and notation from
\cite{bengioMachineLearningCombinatorial2020} to formalize the algorithm
...
\paragraph{\textit{Algorithm as MDP} framework.}
...
After the definition of the MDP itself, the way we parameterize the class of
admissible policies is the second way of incorporating prior knowledge into the
learning pipeline.
...
\begin{figure}
  ...
  \caption{Two states of the constructive scheduling MDP that are equivalent
    with respect to optimal completion, because they only differ by a fixed
    shift of time. It can be shown that any optimal completion of the above
    partial schedule is also optimal for the lower one.}%
  \label{fig:abstract}
\end{figure}
...
\subsection{Platoon preservation: threshold heuristics}
...
\subsection{Further generalization: recurrent neural embedding}\label{sec:neural-param}
...
The evaluation is based on two aspects. Of course, the quality of the produced
solutions is important. Second, we need to take into account the time that the
algorithm requires to compute the solutions. We need to be careful here, because
some of our methods involve both training time as well as inference time.
...
to~\eqref{eq:mixture} with parameters as given in~Table~\ref{tab:distributions}.
...
order, which we compute by solving the MILP problem~\eqref{eq:C'}, where we use the two
cutting planes of~\Cref{sec:cuts}. We use a fixed time limit of two minutes per
instance for the integer programming, in order to keep the total analysis time
within reason. Therefore, we might not always be able to obtain the actual
$\eta^{*}$ some of the larger instances; we use the best solution found by the
solver as a proxy. Given some $\eta$, we define its \textit{optimality gap} as
\begin{align*}
...
In Table~\ref{tab:heuristics-results}, we compare the performance of integer programming with the three
heuristics, being the \emph{threshold heuristic learned with grid search}, the
\emph{recurrent neural embedding parameterized policy with imitation learning}
and \emph{neural parameterized policy with policy-gradient optimization}.
%
The latter does not use our hand-crafted recurrent neural network
parameterization, but instead uses the default policy network of the stable
baselines PPO
implementation\footnote{\url{https://stable-baselines3.readthedocs.io/en/master/modules/ppo.html}}.

\paragraph{Interpretation of results.}

The threshold rule clearly provide a heuristic that is cheap to fit and that works
well for most problem distributions; there is one outlier of almost 11\%, which
we cannot really explain.

As expected, the imitation learning heuristic performs always better than
reinforcement learning (PPO), because it has access to more high-quality data.
%
It becomes increasingly expensive to compute this expert demonstration for
larger instances, which is why we did not include imitation learning results for
the largest two problem distributions.
%
Although PPO provides a ready-to-use method for distributions with instances of
any size, its performance clearly degrades for the larger instances.
%
We hypothesize that our recurrent embedding could provide a better alternative
to the default policy network of the stable baselines implementation. There was
not enough time to try this during the current project.

We stress again that each of the heuristics has an evaluation time of less than a second. In other words, we pay the price of the training time once, after which the learned fast heuristic can be applied nearly instantly to any unseen instance.
%
Integer programming, on the other hand, does not benefit from this learning,
which means that it takes the indicated average time for unseen instances.

\begin{table}[t]
  \caption{\emph{Performance comparison of heuristics with respect to integer programming baseline.}
    ...
    %
    To evaluate the integer programming baseline (MILP), we sample and solve 10 instances to optimality, with a fixed time limit of five minutes per instance.
    For each distribution, we indicate the average objective (opt) and the average solver time ($T$).
    %
    For each heuristic, we list the average objective for the 10 test instances (obj), the average optimality gap with respect to the optimal MILP solutions (gap) and the total training time ($T_\mathrm{train}$).
    %
    The lowest gap is highlighted in each row.
    %
    The evaluation time of heuristics is not listed, because it is less than 1
...
\subsection{Why is stochasticity useful?}

% stochastic transitions
It might not be immediately clear why we would want to introduce stochasticity in
...
%
This has for example been done in existing joint methods, in which some part of
...
We argue that---in principle---these assumptions can be used to reduce the
...
%
Again, we aim to solve this problem using a constructive heuristic.
%
% Quite a few existing works have adopted the same strategy and applied deep
...
job-shop-like problem hinges on this assumption, because we essentially
...
\chapter{Job shop scheduling and disjunctive graph}\label{app:job-shop}
...
\chapter{Local search for crossing time scheduling}\label{sec:local_search}
