...
\date{}
\newcommand{\mytitle}{\huge \bf Coordination of Autonomous Traffic in Networks of Intersections using Machine Learning-Based Scheduling}
...
We now present a concise discussion of the general ML for CO methodology,
centered around the follow three aspects.
%
We start by explaining that generalization should be regarded as the central
goal.
%
The point is to make sure that policies generalize. When they do, this also
makes up for the longer traning of the model, which can happen offline.
%
After that, we discuss the two most important engineering-type aspects, which
are the way in which the policy is parameterized and the corresponding
...
% explicit dependence on problem specification s=(\mathcal{N},a)
...
% formal description
% We now make the above more precise by providing the formal MDP specification.
\subsection{Constructive scheduling MDP}\label{sec:MDP}

% \begin{itemize}
%   \item input = instance $s = (\mathcal{N}, a) \in \mathcal{I}$
%   \item initial state = $\mathcal{G}_0(s)$ as a function of instance
%   \item states are disjunctive graphs $\mathcal{G}_t$
%   \item actions are routes $\mathcal{A} = \mathcal{R}$
...
We will now precisely define the MDP that models the constructive scheduling
procedure.

% each instance s defines an MDP \mathcal{M}(s)


% instance -> empty disjunctive graph
Each instance $s$ corresponds uniquely to an empty disjunctive graph
$\mathcal{G}_0 = \mathcal{G}_0(s)$.
%
All vehicles are said to be \emph{unscheduled} in $\mathcal{G}_0$, which serves
as the initial state of the MDP.
% empty disjunctive graph is initial state
Given some instance $s$, the possible states of the MDP correspond to all
possible disjunctive graphs that can be obtained by adding disjunctive arcs to
$\mathcal{G}_0$.
%
To help the following presentation, we will use $k(\mathcal{G}, r)$ to denote
the number of vehicles on route $r$ that are \emph{scheduled} in state $\mathcal{G}$,
so initially we have $k(\mathcal{G}_0, r) = 0$ for all $r \in \mathcal{R}$.
%
In each step, we have to choose some route $r$ which has still unscheduled
vehicles left, so the route indices form the action space
$\mathcal{A} = \mathcal{R}$.
%
We will now explain how the state transitions happen.
%
Let $\mathcal{G}$ denote the current state, and suppose we choose action $r$,
which is thus valid as long as $k(\mathcal{G}, r) < n_r$.
%
Observe that the first unscheduled vehicle on route $r$ is
\begin{align}
  i = (r, k(\mathcal{G}, r) + 1).
\end{align}
%
We add vehicle $i$ to the partial schedule by adding all the disjunctive arcs
from $i$ to each unscheduled vehicle on all other routes, so formally these arcs
are
\begin{align}
  \big\{ i \rightarrow (q, k) : q \neq r, \;  k \in \{ k(\mathcal{G}, q) + 1, \dots, n_q \} \big\} .
\end{align}
%
After adding these arcs to $\mathcal{G}$, we obtain the next state $\mathcal{G}'$.
%
We emphasize that all transitions are thus defined deterministically, because
the process is so simple that we do not need to introduce stochasticity in the
model to approximate complex computations.
\comment{Introduce additional notation, if neccessary. For example, introduce
  $\eta(\mathcal{G})$ (or with sequence notation $\eta_{1:t} = \eta(\mathcal{G}_t)$) as
  the partial route sequence belonging to the partial disjunctive graph. This
  can be interpreted as a sort of \emph{read-out} function to obtain the solution from
  the final state. Prove that the complete disjunctive graph $\mathcal{G}_N$ is
  always acyclic and complete, such that $y(\eta(\mathcal{G}_N))$ is a valid active
  schedule. [ optional: example MDP with example transitions ]}

% episode length, final state and reward
It can be shown that, after $N$ transitions of the automaton, we obtain some
acyclic complete disjunctive graph $\mathcal{G}_N$. Therefore, it corresponds to
some complete selection of disjunctive arcs $\mathcal{O}_N$, and we can define
$y(\mathcal{G}_N) = y(\mathcal{O}_N)$ to be the corresponding active schedule.
Furthermore, we can define a single final reward equal to the negative sum of
all crossing times
\begin{align}
  r_f(\mathcal{G}_N) := \sum_{i \in \mathcal{N}} y_i(\mathcal{G}_N) ,
\end{align}
where we use the subscript $f$ to avoid confusion with the use of $r$ for routes
and to distinguish it from an equivalent reward that we will define later.

\subsection{Auxiliary features and dense rewards}

Given the MDP above, we now turn our attention to parameterizing the policy $\pi$.
%
...
the state.
%
From here on, we will write states as $\mathcal{G}_t$, indicating at which step
$t \in \{0, \dots, N\}$ it occurs. With this notation, $\mathcal{G}_0$ is an
initial state and $\mathcal{G}_N$ a final state.
%
We will use the subscript $t$ to emphasize that derived quantities depend on
some the current state $\mathcal{G}_t$.
%
For example, we will write the unscheduled vehicle counts used above as
$k_t(r) := k(\mathcal{G}_t, r)$.
%
