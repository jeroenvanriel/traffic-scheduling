...
\begin{block}{\small Q1: Exploit problem structure (decomposition)}
\footnotesize \justifying How can we model the offline trajectory optimization
...
\end{block}
\begin{block}{\small Q2: Automatically derive heuristics (learning)}
\footnotesize \justifying Can recent deep reinforcement learning formulations for
