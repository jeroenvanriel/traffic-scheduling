...
however, vehicles need to be delayed by some amount $d_{j} \geq 0$ in order to
meet some safety constraints. We consider the following two types of
constraints.
...
a minimum amount of time $p_{j}$ after the moment vehicle $j$ crosses, so the first type
of constraint is given by
\begin{align}
...
to guarantee safety, we require an additional minimum amount of time $s$ between
the crossing moments, so the second type of constraint is given by
\begin{align}
  \label{eq:traffic-constr-2}
  y_{j} + p_{j} + s \leq y_{l} \; \text{ or } \; y_{l} + p_{l} + s \leq y_{j} .
\end{align}
This contraint features a choice, because the order of the two vehicles is free
to choose by the traffic controller.

% feasible schedule
It is the task of the traffic controller to determine $y$ that satisfies these
constraints, while optimizing some kind of performance metric.
% total delay objective
The difference $y_{j} - r_{j}$ between the earliest crossing time and the actual
...
vehicles. That is, the continuous control of the speed of each vehicle is not
yet determined. For the current project, we assume that a safe trajectory can be
...
modelling elements from the scheduling literature, before restating the problem
in this framework.
...
\begin{eg}
  \label{eg:non-delay}
  Consider $n=2$ jobs with processing times $p_{1}=2, p_{2}=1$ and
  release dates $r_{1}=0, r_{2}=\epsilon > 0$. Processing job 1 before job 2
  results in a schedule with total completion time $\sum C_{j} = 2 + 3 = 5$.
  However, scheduling job 2 first requires us to introduce $\epsilon$ idle time,
  but has a better total completion time of
  $\sum C_{j} = (\epsilon + 1) + (\epsilon + 1 + 2) = 4 + 2 \epsilon$, for
  $\epsilon < 1/2$. See Figure~\ref{fig:non-delay} for an illustration of both schedules.
\end{eg}
...
\begin{figure}
  ...
  \caption{Illustration of the two possible schedules for
    Example~\ref{eg:non-delay}. The unforced idle time in the second schedule is
    shaded.}
  \label{fig:non-delay}
\end{figure}
...
of jobs $j_{1} \in F_{k_{1}}$, $j_{2} \in F_{k_{2}}$ must satisfy either
\begin{align*}
y_{j_{2}} \geq C_{j_{1}} + s_{k_{1},k_{2}} ,
\end{align*}
is satisfied or
\begin{align*}
y_{j_{1}} \geq C_{j_{2}} + s_{k_{2},k_{1}} .
\end{align*}
...
% constraints
Let $n_{k}$ denote the total number of vehicles arriving to lane $k$.
For each lane, we define a familiy $F_{k}$ containing all vehicles that arrive
to it. We assume, without loss of generality, that the jobs are ordered
according to non-decreasing release dates. Therefore, the first type of
constraints~\eqref{eq:traffic-constr-1} for each lane are modelled by the chain
of precedence constraints by assuming that
\begin{align*}
...
experience by all vehicles is equivalent to minimzing the total completion time,
...
would be interesting to verify the complexity in case of a fixed number of
lanes.


\subsubsection{Branch-and-Bound}

A common method of solving combinatorial problems in general and scheduling problems in particular is by formulating it as a Mixed-Integer Linear Program.

\vspace{0.5em}
\noindent
\textit{\color{blue}Argue that imposing maximum vehicle delay allows us to limit the set of
disjunctions that we need to consider.} \\
...
\subsubsection{Single Platoon Positioning}
