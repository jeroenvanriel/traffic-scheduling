...
$p=(i_{1},\dots,i_{L})$ as a substring of length $L$ of both routes (including
external nodes). We refer to the first node $i_{1}$ of a common path as a
\textit{merge point}. The set of all common paths for vehicles $j$ and $l$ is
denoted by $P_{jl}$. The set of all merge points for vehicles $j$ and $l$ is
denoted by $M_{jl}$.

\vspace{0.5em}
\noindent
\textit{\color{blue}Illustrate common paths and merge points with a small
  example (figure).}
\vspace{0.5em}
