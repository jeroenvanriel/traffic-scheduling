...
operation $O_{jk}$, we denote the starting time as $y(O_{jk})$. A valid schedule
is given by setting values for $y_{ij}$ such that the above requirements are
met. There are various measures for how \textit{good} a given schedule is. For
the purpose of the discussion that follows, let us consider the well-known
makespan objective
\begin{align}
\max_{(i,j) \in N} C_{ij},
\end{align}
...
distinct jobs $j$ and $l$ that need to be processed on the same machine $i$,
...
\subsubsection{Augmented disjunctive graph}

In addition to encoding operation ordering decisions, the disjunctive graph can
also be used to compute lower bounds on the completion times by extending it as
follows. We add a \textit{source} $S$ and a \textit{sink} $T$, which are dummy
nodes. The source has outgoing arcs with weight zero to every first operation of
each job. Every last operation of each job is connected to the sink. Each
outgoing arc of a node $(i,j)$ is set to have weight equal to $p_{ij}$.

Given a complete disjunctive graph, we can now recursively calculate a lowerbound $LB(O_{jk})$ on the completion time of operation $O_{jk}$ from the following relationship
\begin{align}
...
\vspace{0.5em}
\noindent
\textit{\color{blue}explain disjunctive graph in case of generalized prec. constraints}
