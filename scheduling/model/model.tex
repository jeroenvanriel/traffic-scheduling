...
\begin{itemize}
  \item simple situations with total completion time in which some kind of LPT rule hold
  \item tandem networks where $\sum C_{j}$ causes platoon splitting when there is ``delay propagation''
  \item can we formalise such ``delay propagation'' with a graph?
\end{itemize}
